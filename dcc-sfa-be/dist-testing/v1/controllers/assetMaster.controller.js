    ...
    async getAllAssetMaster(req, res) {
        try {
            ...
            if (statusLower === 'active')
                andConditions.push({ is_active: 'Y' });
            if (statusLower === 'inactive')
                andConditions.push({ is_active: 'N' });
            if (outlet_id)
            ...
            if (isScopeRestricted) {
                if (depotIds.length > 0) {
                    if (depot_id) {
                        const parsedDepotId = parseInt(depot_id, 10);
            ...
            const statsAndConditions = andConditions.filter((c) => !c.is_active);
            const statsFilter = statsAndConditions.length > 0 ? { AND: statsAndConditions } : {};
