...
const register = async (req, res) => {
  try {
    ...
    const existing = await prisma_client_1.default.users.findFirst({
      where: { email },
    });
    if (existing) {
...
const login = async (req, res) => {
  try {
    ...
    if (!user) {
      console.log(
        `Failed login attempt for unknown user: ${identifier} from IP: ${(0, ipUtils_1.getClientIP)(req)}`
      );
    ...
    if (platform === 'mobile') {
      const existingSession =
        await prisma_client_1.default.api_tokens.findFirst({
          where: {
            user_id: user.id,
            is_active: 'Y',
            is_revoked: false,
...
const forgotPassword = async (req, res) => {
  try {
    ...
    const userAgent = req.get('User-Agent') || 'Unknown';
    const otpStored = await (0, otp_util_1.storeOTP)(
      email,
      otpCode,
      ipAddress,
      userAgent,
