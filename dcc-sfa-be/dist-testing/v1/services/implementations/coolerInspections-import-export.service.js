...
exports.CoolerInspectionsImportExportService = void 0;
...
class CoolerInspectionsImportExportService extends import_export_service_1.ImportExportService {
    modelName = 'cooler_inspections';
    displayName = 'Cooler Inspections';
    uniqueFields = ['id'];
    searchFields = ['issues', 'action_taken'];
    columns = [
        {
            key: 'cooler_id',
            header: 'Cooler ID',
            ...
            validation: value => {
                if (!value)
                    return 'Cooler ID is required';
                const id = parseInt(value);
                if (isNaN(id) || id <= 0)
                    return 'Cooler ID must be a positive number';
                return true;
            },
            transform: value => parseInt(value),
            description: 'ID of the cooler to inspect (required)',
    ...
    async getSampleData() {
        ...
        const coolerId1 = coolers[0]?.id || 999;
        const coolerId2 = coolers[1]?.id || 999;
        const coolerId3 = coolers[2]?.id || 999;
        const userId1 = users[0]?.id || 999;
        const userId2 = users[1]?.id || 999;
        const visitId1 = visits[0]?.id || null;
        const visitId2 = visits[1]?.id || null;
        return [
            {
                cooler_id: coolerId1,
                inspected_by: userId1,
    ...
    async transformDataForExport(data) {
        return data.map(inspection => {
            ...
            const depot = customer?.customer_depot;
            const coolerType = cooler?.cooler_types;
            const coolerSubType = cooler?.cooler_sub_types;
            const assetMaster = cooler?.cooler_asset_master;
            const inspector = inspection.users;
            const locationType = depot ? 'Depot' : 'Outlet';
            const depotName = depot?.name || '';
            const depotOutletCode = depot ? depot.code : customer?.code || '';
            const outletName = customer?.name || '';
            const formatDate = (d) => d
                ...
                : '';
            return {
                barcode: assetMaster?.barcode || cooler?.code || '',
                serial: cooler?.serial_number || '',
                type: coolerType?.name || assetMaster?.asset_master_asset_types?.name || '',
                sub_type: coolerSubType?.name ||
                    coolerSubType?.code ||
                    assetMaster?.asset_master_asset_sub_types?.name ||
                    assetMaster?.asset_master_asset_sub_types?.code ||
                    '',
                brand: cooler?.brand ||
                    assetMaster?.brand ||
                ...
                depot_outlet_code: depotOutletCode,
                outlet_name: outletName,
                status: cooler?.is_active === 'Y' ? 'Active' : 'Inactive',
                disabled: cooler?.is_active === 'N' ? 'Yes' : 'No',
                install_date: formatDate(cooler?.install_date),
                last_scanned_date: formatDate(cooler?.last_scanned_date),
                last_scanned_seller_code: inspector?.employee_id || '',
                last_scanned_seller_name: inspector?.name || '',
            };
        });
    }
    ...
    async exportToExcel(options = {}) {
        ...
        const where = {};
        const coolerConditions = {};
        if (rawFilters.barcode) {
            coolerConditions.OR = [
                { code: { contains: rawFilters.barcode } },
                { cooler_asset_master: { barcode: { contains: rawFilters.barcode } } },
            ];
        }
        if (rawFilters.serial) {
            coolerConditions.serial_number = { contains: rawFilters.serial };
        }
        if (rawFilters.brand) {
            coolerConditions.brand = { contains: rawFilters.brand };
        }
        if (rawFilters.location) {
            const loc = rawFilters.location.toLowerCase();
            if (loc === 'depot') {
                coolerConditions.coolers_customers = {
                    depot_id: { not: null },
                };
            }
            else if (loc === 'outlet') {
        ...
        if (rawFilters.depot_name) {
            coolerConditions.coolers_customers = {
                ...coolerConditions.coolers_customers,
                customer_depot: {
                    name: { contains: rawFilters.depot_name },
                },
            };
        }
        if (rawFilters.code) {
            coolerConditions.OR = [
                ...(coolerConditions.OR || []),
                { coolers_customers: { code: { contains: rawFilters.code } } },
                {
                    coolers_customers: {
                        customer_depot: { code: { contains: rawFilters.code } },
                    },
                },
            ];
        }
        if (rawFilters.outlet_name) {
            coolerConditions.coolers_customers = {
                ...coolerConditions.coolers_customers,
                name: { contains: rawFilters.outlet_name },
            };
        }
        if (rawFilters.status) {
            const statusVal = rawFilters.status.toLowerCase();
            coolerConditions.is_active =
                statusVal === 'active' || statusVal === 'y' ? 'Y' : 'N';
        }
        if (rawFilters.disabled) {
            const disabledVal = rawFilters.disabled.toLowerCase();
            coolerConditions.is_active =
                disabledVal === 'yes' || disabledVal === 'true' || disabledVal === 'y'
                    ? 'N'
                    : 'Y';
        }
        const lastScannedDateVal = rawFilters.last_scanned_date;
        if (lastScannedDateVal) {
            const targetDate = new Date(lastScannedDateVal);
            if (!isNaN(targetDate.getTime())) {
                const startOfDay = new Date(targetDate);
                startOfDay.setHours(0, 0, 0, 0);
                const endOfDay = new Date(targetDate);
                endOfDay.setHours(23, 59, 59, 999);
                coolerConditions.last_scanned_date = {
                    gte: startOfDay,
                    lte: endOfDay,
                };
            }
        ...
        if (Object.keys(coolerConditions).length > 0) {
            where.coolers = coolerConditions;
        }
            ...
            include: {
                coolers: {
                    select: {
                        ...
                        is_active: true,
                        cooler_types: { select: { name: true, code: true } },
                        cooler_sub_types: { select: { name: true, code: true } },
                        cooler_asset_master: {
                            select: {
                                barcode: true,
                                brand: true,
                        ...
                        coolers_customers: {
                            select: {
                                ...
                                code: true,
                                type: true,
                                customer_depot: { select: { name: true, code: true } },
                            },
                        },
                    },
                },
                users: { select: { name: true, employee_id: true } },
                visits: { select: { visit_date: true } },
            },
        };
        if (options.limit)
        ...
        const workbook = new ExcelJS.Workbook();
        const worksheet = workbook.addWorksheet(this.displayName);
        // Columns matching the new Excel layout from the provided screenshot
        const exportColumns = [
            { header: 'Barcode', key: 'barcode', width: 20 },
            { header: 'Serial', key: 'serial', width: 20 },
            { header: 'Type', key: 'type', width: 15 },
            { header: 'SubType', key: 'sub_type', width: 15 },
            { header: 'Brand', key: 'brand', width: 15 },
            { header: 'Location (Outlet/Depot)', key: 'location', width: 22 },
            ...
            { header: 'InstallDate', key: 'install_date', width: 22 },
            { header: 'LastScanned Date', key: 'last_scanned_date', width: 22 },
            {
        ...
        exportData.forEach((row, index) => {
            ...
            totalInspections++;
            if (inspection.is_active === 'Y')
                activeInspections++;
            if (inspection.is_active === 'N')
                inactiveInspections++;
            if (inspection.is_working === 'Y')
                workingInspections++;
            if (inspection.is_working === 'N')
                nonWorkingInspections++;
            if (inspection.action_required === 'Y')
...
exports.CoolerInspectionsImportExportService = CoolerInspectionsImportExportService;
//# sourceMappingURL=coolerInspections-import-export.service.js.map