...
exports.mockCustomerTypes = void 0;
exports.seedCustomerType = seedCustomerType;
exports.clearCustomerType = clearCustomerType;
...
exports.mockCustomerTypes = mockCustomerTypes;
async function seedCustomerType() {
    try {
        for (const customerType of mockCustomerTypes) {
            const existingType = await prisma_client_1.default.customer_type.findFirst({
                where: { type_name: customerType.type_name },
            });
            if (!existingType) {
                await prisma_client_1.default.customer_type.create({
                    data: {
...
async function clearCustomerType() {
    try {
        await prisma_client_1.default.customer_type.deleteMany({});
    }
    catch (error) {
        if (error?.code === 'P2003' ||
            error?.message?.includes('Foreign key constraint')) {
            console.warn('⚠️  Could not clear all customer types due to foreign key constraints. Some records may be in use by customers.');
        }
        else {
            throw error;
        }
...
//# sourceMappingURL=customerType.seeder.js.map