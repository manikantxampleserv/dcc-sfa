...
async function seedProducts() {
    try {
        ...
        const subCategories = await prisma_client_1.default.product_sub_categories.findMany({
            select: {
                id: true,
        ...
        const targetGroups = await prisma_client_1.default.product_target_group.findMany({
            select: { id: true, name: true },
        });
        const webOrders = await prisma_client_1.default.product_web_order.findMany({
            select: { id: true, name: true },
        });
        ...
        const shelfLives = await prisma_client_1.default.product_shelf_life.findMany({
            select: { id: true, name: true },
        });
        const categoryMap = new Map(categories.map(cat => [cat.category_name, cat.id]));
        const subCategoryMap = new Map(subCategories.map(sc => [sc.sub_category_name, sc.id]));
        ...
        const otherSubCategories = otherCategoryId
            ...
            : [];
        const defaultSubCategoryId = otherSubCategories.length > 0 ? otherSubCategories[0].id : null;
        for (const product of mockProducts) {
            const existingProduct = await prisma_client_1.default.products.findFirst({
                where: { code: product.code },
            });
            if (existingProduct) {
                continue;
            }
            const categoryId = product.category_name && categoryMap.has(product.category_name)
            ...
            if (product.sub_category_name) {
                ...
                const categorySubCategories = subCategories.filter(sc => sc.product_category_id === categoryId);
                subCategoryId =
            ...
            if (!categoryId || !subCategoryId || !brandId || !defaultUnitId) {
                ...
                if (!defaultUnitId)
                    missingFields.push('unit_of_measurement');
                console.warn(`⚠️  Missing required fields for product: ${product.code} - ${product.name} (missing: ${missingFields.join(', ')})`);
                continue;
            }
            ...
            const shelfLifeId = product.shelf_life_name
                ...
                : null;
            await prisma_client_1.default.products.create({
                data: {
                    name: product.name,
                    code: product.code,
...
async function clearProducts() {
    try {
        ...
        for (const code of productCodes) {
            try {
                await prisma_client_1.default.products.deleteMany({
                    where: { code },
                });
            }
            catch (error) {
                console.warn(`⚠️  Could not delete product with code ${code} due to foreign key constraints. It may be in use.`);
            }
        }
    }
    catch (error) {
        console.warn('⚠️  Error clearing products. Some products may be in use and cannot be deleted.');
