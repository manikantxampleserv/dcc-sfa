 ...
 */
export type Modules = 'executive-dashboard' | 'grading-dashboard' | 'user' | 'role' | 'depot' | 'zone' | 'currency' | 'route' | 'route-type' | 'outlet' | 'outlet-group' | 'asset-type' | 'asset-brand' | 'asset-sub-types' | 'asset-master' | 'vehicle' | 'brand' | 'product-category' | 'product-sub-category' | 'unit-of-measurement' | 'product' | 'sales-target-group' | 'sales-target' | 'sales-bonus-rule' | 'kpi-target' | 'survey' | 'promotions' | 'order' | 'return' | 'payment' | 'invoice' | 'visit' | 'asset-movement' | 'maintenance' | 'installation' | 'inspection' | 'van-stock' | 'stock-movement' | 'inventory-management' | 'competitor' | 'customer-complaint' | 'customer-category' | 'customer-type' | 'customer-channel' | 'product-flavour' | 'product-volume' | 'product-shelf-life' | 'product-type' | 'product-target-group' | 'product-web-order' | 'inventory-items' | 'location' | 'route-effectiveness' | 'report' | 'approval' | 'exception' | 'tax-master' | 'login-history' | 'token' | 'setting' | 'templates' | 'region' | 'district' | 'city' | 'pricelist' | 'audit-log' | 'attendance' | 'request' | 'reconciliation' | 'settlement-sheet';
/**
 * @type Actions
