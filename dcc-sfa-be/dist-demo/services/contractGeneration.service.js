...
class ContractGenerationService {
    async generateCoolerIssuanceContract(assetMovementId) {
        ...
        return new Promise((resolve, reject) => {
            ...
            const startX = 40;
            const issuedTo = assetMovement.asset_movement_to_customer?.name ||
                assetMovement.asset_movement_to_depot?.name ||
                'N/A';
            const fromName = assetMovement.asset_movement_from_customer?.name ||
                assetMovement.asset_movement_from_depot?.name ||
                'N/A';
            const locationName = assetMovement.asset_movement_to_depot?.name ||
