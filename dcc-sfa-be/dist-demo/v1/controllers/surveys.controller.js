...
const serializeSurvey = (survey) => ({
    ...
    roles: survey.surveys_roles
        ...
        : null,
    target_products: survey.survey_products?.map((sp) => sp.product_id) || [],
    fields: survey.survey_fields?.map((field) => ({
        id: field.id,
    ...
    depots: survey.survey_depots?.map((item) => item.depot_id) || [],
    zones: survey.survey_zones?.map((item) => item.zone_id) || [],
    routes: survey.survey_routes?.map((item) => item.route_id) || [],
    outlets: survey.survey_customers?.map((item) => item.customer_id) || [],
    customer_types: survey.survey_customer_types?.map((item) => item.customer_type_id) ||
        [],
    customer_categories: survey.survey_customer_categories?.map((item) => item.customer_category_id) || [],
    customer_channels: survey.survey_customer_channels?.map((item) => item.customer_channel_id) || [],
});
exports.surveysController = {
    async createOrUpdateSurvey(req, res) {
        const data = req.body;
        ...
        try {
            const result = await prisma_client_1.default.$transaction(async (tx) => {
                ...
                const payload = {
                    ...
                    published_at: surveyData.published_at && surveyData.published_at.trim() !== ''
                        ? new Date(surveyData.published_at)
                        : null,
                    expires_at: surveyData.expires_at && surveyData.expires_at.trim() !== ''
                ...
                const processFields = async (items, surveyId, tx, parentFieldId = null) => {
                    const processedIds = [];
                    for (let index = 0; index < items.length; index++) {
                        ...
                        if (Array.isArray(field.child_fields) &&
                            field.child_fields.length > 0) {
                            const childIds = await processFields(field.child_fields, surveyId, tx, savedFieldId);
                ...
                if (Array.isArray(surveyData.target_products)) {
                    ...
                    if (surveyData.target_products.length > 0) {
                        await tx.survey_products.createMany({
                            data: surveyData.target_products.map((pid) => ({
                                survey_id: survey.id,
                                product_id: pid,
                            })),
                        });
                ...
                if (Array.isArray(surveyData.depots)) {
                    ...
                    if (surveyData.depots.length > 0) {
                        await tx.survey_depots.createMany({
                            data: surveyData.depots.map((depotId) => ({
                                parent_id: survey.id,
                                depot_id: depotId,
                                is_active: 'Y',
                                createdby: userId,
                ...
                if (Array.isArray(surveyData.zones)) {
                    ...
                    if (surveyData.zones.length > 0) {
                        await tx.survey_zones.createMany({
                            data: surveyData.zones.map((zoneId) => ({
                                parent_id: survey.id,
                                zone_id: zoneId,
                                is_active: 'Y',
                                createdby: userId,
                ...
                if (Array.isArray(surveyData.routes)) {
                    ...
                    if (surveyData.routes.length > 0) {
                        await tx.survey_routes.createMany({
                            data: surveyData.routes.map((routeId) => ({
                                parent_id: survey.id,
                                route_id: routeId,
                                is_active: 'Y',
                                createdby: userId,
                ...
                if (Array.isArray(surveyData.outlets)) {
                    ...
                    if (surveyData.outlets.length > 0) {
                        await tx.survey_customers.createMany({
                            data: surveyData.outlets.map((customerId) => ({
                                parent_id: survey.id,
                ...
                if (Array.isArray(surveyData.customer_types)) {
                    ...
                    if (surveyData.customer_types.length > 0) {
                        await tx.survey_customer_types.createMany({
                            data: surveyData.customer_types.map((typeId) => ({
                                parent_id: survey.id,
                ...
                if (Array.isArray(surveyData.customer_categories)) {
                    ...
                    if (surveyData.customer_categories.length > 0) {
                        await tx.survey_customer_categories.createMany({
                            data: surveyData.customer_categories.map((catId) => ({
                                parent_id: survey.id,
                ...
                if (Array.isArray(surveyData.customer_channels)) {
                    ...
                    if (surveyData.customer_channels.length > 0) {
                        await tx.survey_customer_channels.createMany({
                            data: surveyData.customer_channels.map((chanId) => ({
                                parent_id: survey.id,
            ...
            const finalSurvey = result.finalSurvey;
    ...
    async getAllSurveys(req, res) {
        try {
            ...
            const { data, pagination } = await (0, paginate_1.paginate)({
                model: prisma_client_1.default.surveys,
                filters,
                page: pageNum,
                limit: limitNum,
            ...
            const [totalSurveys, publishedSurveys, draftSurveys, activeSurveys, totalResponses,] = await Promise.all([
                prisma_client_1.default.surveys.count(),
                prisma_client_1.default.surveys.count({ where: { is_published: true } }),
                prisma_client_1.default.surveys.count({ where: { is_published: false } }),
                prisma_client_1.default.surveys.count({ where: { is_active: 'Y' } }),
                prisma_client_1.default.survey_responses.count(),
            ]);
            const categories = await prisma_client_1.default.surveys.groupBy({
                by: ['category'],
            });
            res.success('Surveys retrieved successfully', data.map((s) => serializeSurvey(s)), 200, pagination, {
    ...
    async getSurveyById(req, res) {
        try {
            const { id } = req.params;
            const survey = await prisma_client_1.default.surveys.findUnique({
                where: { id: Number(id) },
                include: {
                    surveys_roles: true,
                    survey_products: true,
    ...
    async deleteSurvey(req, res) {
        try {
            ...
            if (!existingSurvey)
                return res.status(404).json({ message: 'Survey not found' });
            const responseCount = await prisma_client_1.default.survey_responses.count({
    ...
    async publishSurvey(req, res) {
        try {
            ...
            if (!existingSurvey)
                return res.status(404).json({ message: 'Survey not found' });
            if (!existingSurvey.is_published &&
                existingSurvey.survey_fields.length === 0) {
                return res.status(400).json({
                    message: 'Cannot publish survey without fields',
                });
            }
            const survey = await prisma_client_1.default.surveys.update({
                where: { id: Number(id) },
                data: {
    ...
    async getSurveyFields(req, res) {
        try {
            const { surveyId } = req.params;
            const fields = await prisma_client_1.default.survey_fields.findMany({
    ...
    async duplicateSurvey(req, res) {
        try {
            const { id } = req.params;
            const userId = req.user?.id || 1;
            const originalSurvey = await prisma_client_1.default.surveys.findUnique({
                where: { id: Number(id) },
                include: {
                    survey_fields: {
                        orderBy: { sort_order: 'asc' },
            ...
            if (!originalSurvey)
                return res.status(404).json({ message: 'Survey not found' });
            const newSurvey = await prisma_client_1.default.surveys.create({
                data: {
