...
/**
 * Function to serializeVanInventory
 * @param item
 */
/**
 * serializeVanInventory
 */
const serializeVanInventory = (item) => {
    ...
    return {
        ...
        sub_inventory_users: item.van_inventory_sub_users
            ? item.van_inventory_sub_users.map((su) => ({
...
/**
 * Function to getAvailableBatchesForProduct
 * @param tx
 * @param productId
 * @param loadingType
 */
...
/**
 * Function to updateInventoryStock
 * @param tx
 * @param productId
 * @param locationId
 * @param quantity
 ...
 */
async function updateInventoryStock(tx, productId, locationId, quantity, loadingType, batchId, serialId, userId, vanUserId, baseQuantity = 0) {
...
/**
 * Function to createStockMovement
 * @param tx
 * @param data
 */
/**
...
/**
 * Function to updateSubUsersInventoryStock
 * @param tx
 * @param inventoryData
 * @param productId
 * @param quantity
...
/**
 * Function to normalizeInventoryItemSerials
 * @param item
 */
/**
 * normalizeInventoryItemSerials
 */
...
/**
 * Function to processApprovedVanInventoryStock
 * @param inventoryId
 * @param userId
 * @param requestData
 */
/**
 * processApprovedVanInventoryStock
 */
async function processApprovedVanInventoryStock(inventoryId, userId, requestData) {
    console.log(` processApprovedVanInventoryStock starting for inventory ID: ${inventoryId} `);
    ...
    await prisma_client_1.default.$transaction(async (tx) => {
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            if (!productId) {
                console.log('   Skipping: no product_id and product not found by sap_code');
                continue;
            }
            ...
            if (loadingType === 'L') {
                ...
                if (trackingType === 'BATCH') {
                    ...
                    for (let b = 0; b < batchData.length; b++) {
                        ...
                        console.log('  Batch quantity:', batchQty, 'Base Quantity:', batchBaseQty);
                        ...
                        if (!existingVanItem) {
                            ...
                            if (unlinkedVanItem) {
                                ...
                                console.log(`  Linked existing unlinked van_inventory_item (ID: ${unlinkedVanItem.id}) to batch_lot: ${batchLot.id}`);
                            }
                        }
                        if (existingVanItem) {
                            console.log('  Found existing van_inventory_item in current inventory, skipping quantity update after approval.');
                        }
                        else {
                        ...
                        await updateInventoryStock(tx, product.id, inventoryLocationId, batchQty, 'L', batchLot.id, null, userId, inventoryUserId, batchBaseQty);
                    ...
                    console.log('  Normalized serial data:', JSON.stringify(serialData, null, 2));
                    ...
                    for (let s = 0; s < serialData.length; s++) {
                        ...
                        console.log('  Found existing serial?', !!existingSerial, existingSerial ? `status: ${existingSerial.status}` : 'n/a');
                        if (existingSerial) {
                        ...
                        if (!existingVanItem) {
                            ...
                            if (unlinkedVanItem) {
                                ...
                                console.log(`  Linked existing unlinked serial van_item (ID: ${unlinkedVanItem.id}) to serial: ${existingSerial.id}`);
                            }
                        }
                        if (existingVanItem) {
                            console.log('  Found existing van item for serial, skipping quantity update after approval.');
                        }
                        else {
                    ...
                    if (existingVanItem) {
                        console.log('  Found existing van item, skipping quantity update after approval.');
                    }
                    else {
                        await tx.van_inventory_items.create({
                            data: {
                    ...
                    await updateInventoryStock(tx, product.id, inventoryLocationId, qty, 'L', null, null, userId, inventoryUserId, baseQty);
                    await createStockMovement(tx, {
                        product_id: product.id,
                        batch_id: null,
                        serial_id: null,
                ...
                if (trackingType === 'BATCH') {
                    ...
                    for (let b = 0; b < batchData.length; b++) {
                        const batchInput = batchData[b];
                        console.log(`  --- Processing unload batch ${b + 1}/${batchData.length} ---`);
                        console.log('  Batch input:', JSON.stringify(batchInput, null, 2));
                        const batchQty = parseInt(batchInput.quantity, 10) || 0;
                        const batchBaseQty = parseInt(batchInput.base_quantity, 10) || 0;
                        if (batchQty <= 0 && batchBaseQty <= 0) {
                        ...
                        console.log('  Batch quantity to unload:', batchQty, 'Base Quantity:', batchBaseQty);
                        let batchLot = null;
                        ...
                        if (inventoryStock) {
                            console.log('   Updating inventory_stock: current_stock', inventoryStock.current_stock, '→', Math.max(0, (inventoryStock.current_stock || 0) - batchQty));
                            await tx.inventory_stock.update({
                                ...
                                data: {
                                    ...
                                    base_quantity: Math.max(0, (inventoryStock.base_quantity || 0) - batchBaseQty),
                                    updatedate: new Date(),
                                    updatedby: userId,
                                },
                            });
                    ...
                    for (let s = 0; s < serialData.length; s++) {
                        const serialInput = serialData[s];
                        console.log(`  --- Processing unload serial ${s + 1}/${serialData.length} ---`);
                        console.log('  Serial input:', JSON.stringify(serialInput, null, 2));
                        const serialNumber = typeof serialInput === 'string'
                            ? serialInput
                            : serialInput?.serial_number;
                        ...
                        console.log('  Found serial:', existingSerial.id, existingSerial.serial_number);
                        ...
                        if (!vanItem) {
                            console.log('   Skipping: van_inventory_item not found for this serial');
                            continue;
                        }
                        ...
                        if (inventoryStock) {
                            console.log('   Updating inventory_stock: current_stock', inventoryStock.current_stock, '→', Math.max(0, (inventoryStock.current_stock || 0) - 1));
                            await tx.inventory_stock.update({
                                where: { id: inventoryStock.id },
                                data: {
                    ...
                    if (inventoryStock) {
                        console.log('   Updating inventory_stock: current_stock', inventoryStock.current_stock, '→', Math.max(0, (inventoryStock.current_stock || 0) - qty));
                        await tx.inventory_stock.update({
                            where: { id: inventoryStock.id },
                            data: {
                                current_stock: Math.max(0, (inventoryStock.current_stock || 0) - qty),
                                available_stock: Math.max(0, (inventoryStock.available_stock || 0) - qty),
                                base_quantity: Math.max(0, (inventoryStock.base_quantity || 0) - baseQty),
                                updatedate: new Date(),
                                updatedby: userId,
                            },
                        });
...
exports.vanInventoryController = {
    ...
    async getSalespersonInventoryItems(req, res) {
        try {
            ...
            const stagedBatches = new Set(stagedStocks
                .map(s => s.batch_id)
                .filter((id) => id !== null));
            const stagedSerials = new Set(stagedStocks
                .map(s => s.serial_number_id)
                .filter((id) => id !== null));
            const stagedProductIds = new Set(stagedStocks
            ...
            const itemsRaw = (await prisma_client_1.default.van_inventory_items.findMany({
                where: {
                    parent_id: { in: vanIds },
                    ...(product_id
            ...
            const filteredItems = itemsRaw.filter((it) => {
                if (it.serial_id && stagedSerials.has(it.serial_id))
                    return false;
                if (it.batch_lot_id && stagedBatches.has(it.batch_lot_id))
                    return false;
                if (!it.serial_id &&
                    !it.batch_lot_id &&
            ...
            const data = paginated.map((it) => ({
                ...
                product_name: it.van_inventory_items_products?.name || it.product_name || null,
                product_code: it.van_inventory_items_products?.code || null,
                tracking_type: it.van_inventory_items_products?.tracking_type || null,
                unit: it.unit || null,
                unit_of_measurement: it.van_inventory_items_products?.product_unit_of_measurement,
     ...
     * @param res
     */
    async getSalespersonInventoryItemsDropdown(req, res) {
        try {
            const { salesperson_id } = req.params;
            ...
            const filteredItems = items.filter((it) => {
                if (it.serial_id && stagedSerials.has(it.serial_id))
                    return false;
                if (it.van_inventory_items_batch_lot?.id &&
    ...
    async createOrUpdateVanInventory(req, res) {
        const data = req.body;
        console.log(' createOrUpdateVanInventory INCOMING DATA ', JSON.stringify(data, null, 2));
        const userId = req.user?.id || 1;
        const { van_inventory_items, inventoryItems, ...inventoryData } = data;
        const items = van_inventory_items || inventoryItems || [];
        console.log(' Extracted items for processing ', JSON.stringify(items, null, 2));
        ...
        try {
            const result = await prisma_client_1.default.$transaction(async (tx) => {
                ...
                const resolveRecordByIdOrSap = async (model, idValue, sapValue, entityLabel) => {
                    const normalizedId = idValue !== undefined && idValue !== null && idValue !== ''
                        ? Number(idValue)
                        : null;
                    const normalizedSap = sapValue !== undefined && sapValue !== null && sapValue !== ''
                    ...
                    if (normalizedId || normalizedSap) {
                        throw new Error(`${entityLabel} ${normalizedId ?? normalizedSap} not found`);
                    }
                    return null;
                };
                const userIdentifier = inventoryData.user_id ?? inventoryData.user_sap;
                if (!userIdentifier) {
                    throw new Error('user_id or user_sap is required');
                }
                const spUser = await resolveRecordByIdOrSap('users', inventoryData.user_id, inventoryData.user_sap, 'User');
                if (!spUser) {
                ...
                if (isUpdate && inventoryId) {
                    ...
                    if (inventoryData.sale_type === 'container' &&
                        Array.isArray(inventoryData.sub_inventory_user_ids)) {
                        const subUsersData = inventoryData.sub_inventory_user_ids.map((subUserId) => ({
                    ...
                    for (const item of items) {
                        ...
                        if (trackingType === 'BATCH') {
                            ...
                            for (const batchInput of batchData) {
                                ...
                                console.log(`  Saved batch item: ${product.name} (batch: ${batchInput.batch_number}, qty: ${batchQty})`);
                            }
                        }
                        else if (trackingType === 'SERIAL') {
                            const serialData = normalizeInventoryItemSerials(item);
                            ...
                            for (const serialInput of serialData) {
                                ...
                                console.log(`  Saved serial item: ${product.name} (serial: ${serialNumber})`);
                            }
                        }
                        else {
                            await tx.van_inventory_items.create({
                            ...
                            console.log(`  Saved plain item: ${product.name} (qty: ${qty})`);
                        }
                    }
                        ...
                        if (loadingType === 'L') {
                            if (trackingType === 'BATCH') {
                                ...
                                for (const batchInput of batchData) {
                                    ...
                                    await updateInventoryStock(tx, product.id, inventoryData.location_id || null, batchQty, 'L', batchLot.id, null, userId, inventoryData.user_id, parseInt(batchInput.base_quantity, 10) || 0);
                                ...
                                for (const serialInput of serialData) {
                                    ...
                                    await updateInventoryStock(tx, product.id, inventoryData.location_id || null, 1, 'L', null, existingSerial.id, userId, inventoryData.user_id, 1);
                                ...
                                await updateInventoryStock(tx, product.id, inventoryData.location_id || null, qty, 'L', null, null, userId, inventoryData.user_id, parseInt(item.base_quantity, 10) || 0);
                            ...
                            if (trackingType === 'BATCH') {
                                ...
                                for (const batchInput of batchData) {
                                    ...
                                    await updateSubUsersInventoryStock(tx, inventoryData, product.id, batchQty, 'U', batchLot.id, null, userId, {
                                        movement_type: 'VAN_UNLOAD',
                                        van_inventory_id: inventory.id,
                                ...
                                for (const serialInput of serialData) {
                                    ...
                                    await updateSubUsersInventoryStock(tx, inventoryData, product.id, 1, 'U', null, existingSerial.id, userId, {
                                        movement_type: 'VAN_UNLOAD',
                                        van_inventory_id: inventory.id,
                                ...
                                await updateSubUsersInventoryStock(tx, inventoryData, product.id, qty, 'U', null, null, userId, {
                                    movement_type: 'VAN_UNLOAD',
                                    van_inventory_id: inventory.id,
    ...
    async getAllVanInventory(req, res) {
        try {
            const { page, limit, search, status, loading_type, user_id, depot_id, approval_status, time_filter, start_date, end_date, } = req.query;
            const pageNum = parseInt(page, 10) || 1;
            ...
            if (approval_status) {
                if (approval_status.toUpperCase() === 'ALL') {
                    finalApprovalStatus = undefined;
                }
                else {
                    const statuses = approval_status
                        .split(',')
                ...
                if (statusLower === 'pending')
                    finalApprovalStatus = 'P';
                else if (statusLower === 'approved')
                    finalApprovalStatus = 'A';
                else if (statusLower === 'rejected')
                    finalApprovalStatus = 'R';
            ...
            const [totalVanInventory, activeVanInventory, inactiveVanInventory, vanInventoryThisMonth,] = await Promise.all([
                ...
                prisma_client_1.default.van_inventory.count({ where: { ...filters, is_active: 'Y' } }),
                prisma_client_1.default.van_inventory.count({ where: { ...filters, is_active: 'N' } }),
                prisma_client_1.default.van_inventory.count({
                    where: {
                        ...filters,
                        createdate: {
    ...
    async updateVanInventory(req, res) {
        try {
            ...
            const updated = await prisma_client_1.default.$transaction(async (tx) => {
                ...
                if (inventoryData.sub_inventory_user_ids !== undefined ||
                    inventoryData.sale_type !== undefined) {
                    await tx.van_inventory_sub_users.deleteMany({
                        where: { parent_id: Number(id) },
                    });
                    const currentSaleType = inventoryData.sale_type !== undefined
                        ? inventoryData.sale_type
                        : inv.sale_type;
                    const userIds = inventoryData.sub_inventory_user_ids || [];
                    if (currentSaleType === 'container' &&
                        Array.isArray(userIds) &&
                        userIds.length > 0) {
                        const subUsersData = userIds.map((subUserId) => ({
                            parent_id: Number(id),
                            user_id: Number(subUserId),
                            createdby: userId,
                            log_inst: 1,
            ...
            const vanInventoryItem = await prisma_client_1.default.van_inventory_items.update({
                ...
                data: {
                    ...
                    discount_amount: data.discount_amount !== undefined
                        ...
                        : undefined,
                    tax_amount: data.tax_amount && data.tax_amount !== undefined
    ...
    async getProductBatchDetails(req, res) {
        try {
            ...
            const response = {
                ...
                manufacturing_date: batchLot.manufacturing_date,
                expiry_date: batchLot.expiry_date,
                days_until_expiry: Math.floor((expiry.getTime() - now.getTime()) / (1000 * 60 * 60 * 24)),
    ...
    async getSalespersonInventory(req, res) {
        try {
            const { salesperson_id } = req.params;
            const { page, limit, product_id, document_date, loading_type, include_expired_batches = 'false', batch_status, serial_status, time_filter, } = req.query;
            const pageNum = parseInt(page, 10) || 1;
            const limitNum = parseInt(limit, 10) || 50;
            const processVanInventoryItems = (vanInventories, salesperson) => {
                ...
                for (const vanInventory of vanInventories) {
                    for (const item of vanInventory.van_inventory_items_inventory) {
                        if (product_id &&
                            item.product_id !== parseInt(product_id, 10)) {
                            continue;
                        }
                        ...
                        const serials = product?.serial_numbers_products?.map((serial) => {
                            ...
                            return {
                                ...
                                warranty_days_remaining: serial.warranty_expiry
                                    ? Math.floor((new Date(serial.warranty_expiry).getTime() -
                                        Date.now()) /
                                        (1000 * 60 * 60 * 24))
                                    : null,
                        ...
                        const existingVanInventory = productData.van_inventories.find((vi) => vi.van_inventory_id === vanInventory.id);
                        ...
                        for (const serial of serials) {
                            const existingSerial = productData.serials.find((s) => s.serial_id === serial.serial_id);
                            if (!existingSerial) {
            ...
            if (!salesperson_id ||
                ...
                let overallTotalProducts = new Set();
                let overallTotalVanInventories = new Set();
                ...
                for (const salesperson of allSalespersons) {
                    ...
                    if (vanInventories.length === 0)
                        continue;
                    const { products, totalQuantity, totalRemainingQuantity } = processVanInventoryItems(vanInventories, salesperson);
                ...
                const startIndex = (pageNum - 1) * limitNum;
                const paginatedData = consolidatedSalespersons.slice(startIndex, startIndex + limitNum);
                const pagination = {
                    current_page: pageNum,
                    per_page: limitNum,
                    total_pages: Math.ceil(consolidatedSalespersons.length / limitNum),
                    total_count: consolidatedSalespersons.length,
                    has_next: pageNum < Math.ceil(consolidatedSalespersons.length / limitNum),
                    has_prev: pageNum > 1,
                };
                return res.json({
                    success: true,
            ...
            const vanInventories = await prisma_client_1.default.van_inventory.findMany({
                where: {
                    ...
                    status: 'A',
                    ...(loading_type && {
                        loading_type: loading_type.toUpperCase(),
                    }),
                    ...dateFilter,
                },
                select: {
     ...
     * @param res
     */
    async getinventoryItemSalesperson(req, res) {
        try {
            const { salesperson_id } = req.params;
            const { page, limit, product_id, document_date, include_expired_batches = 'false', batch_status, serial_status, } = req.query;
            const pageNum = parseInt(page, 10) || 1;
            const limitNum = parseInt(limit, 10) || 50;
            const stagedStocks = await prisma_client_1.default.inventory_stock.findMany({
                where: {
            ...
            const processVanInventoryItems = (vanInventories, currentSalespersonId) => {
                let totalQuantity = 0;
                let totalRemainingQuantity = 0;
                ...
                const processedVanInventories = vanInventories
                    ...
                    for (const item of items) {
                        ...
                        const product = item.van_inventory_items_products;
                        const batch = item.van_inventory_items_batch_lot;
                        const trackingType = (product?.tracking_type || 'none').toLowerCase();
                        let shouldSkipItem = false;
                        let allProductBatches = [];
                                ...
                                serials = [
                                    {
                                        ...
                                        warranty_days_remaining: linkedSerial.warranty_expiry
                                            ? Math.floor((new Date(linkedSerial.warranty_expiry).getTime() -
                                                Date.now()) /
                                                (1000 * 60 * 60 * 24))
                                            : null,
                        ...
                        if (serials.length > 0) {
                            serials.forEach((serial) => {
                                if (!productData.serials.find((existing) => existing.serial_id === serial.serial_id)) {
                                    productData.serials.push(serial);
                                    totalSerials++;
                                }
                            });
            ...
            let dateFilter = {};
            const tf = (0, dateFilters_1.getTimeFilter)(req.query.time_filter, req.query.start_date, req.query.end_date);
            if (tf) {
                dateFilter = {
                    OR: [
                        { document_date: tf },
            ...
            if (!salesperson_id ||
                ...
                for (const salesperson of allSalespersons) {
                    ...
                    if (vanInventories.length === 0)
                        continue;
                    const { vanInventories: processedVanInventories, totalProducts, totalQuantity, totalRemainingQuantity, totalBatches, totalSerials, } = processVanInventoryItems(vanInventories, salesperson.id);
                    if (processedVanInventories.length === 0)
                        continue;
                    consolidatedSalespersons.push({
                        salesperson_id: salesperson.id,
                        salesperson_name: salesperson.name,
            ...
            const { vanInventories: processedVanInventories, totalProducts, totalQuantity, totalRemainingQuantity, totalBatches, totalSerials, } = processVanInventoryItems(vanInventories, salespersonIdNum);
            const startIndex = (pageNum - 1) * limitNum;
            const paginatedVanInventories = processedVanInventories.slice(startIndex, startIndex + limitNum);
            const pagination = {
                current_page: pageNum,
                per_page: limitNum,
                total_pages: Math.ceil(processedVanInventories.length / limitNum),
                total_count: processedVanInventories.length,
                has_next: pageNum < Math.ceil(processedVanInventories.length / limitNum),
                has_prev: pageNum > 1,
            };
            res.json({
                success: true,
     ...
     */
    async createVanInventoryFromReconciliation(reconciliationId, approvedByUserId) {
        return prisma_client_1.default.$transaction(async (tx) => {
            ...
            if (!locationId) {
                throw new Error(`Unable to resolve location for reconciliation ${reconciliationId}`);
            }
    ...
    //             number,
    //             { price: number; taxRate: number | null }
    //           >();
    //           for (const productId of productIdsInStock) {
    //             let resolvedPrice: number | null = null;
    //             let resolvedTax: number | null = null;
    //             // 1. Try Depot Pricelist (Special first, then Normal)
    //             if (depotPricelist) {
    //               const depotItem = depotPricelist.pricelist_item.find(
    //                 item => item.product_id === productId
    //               );
    //               if (depotItem) {
    //                 // Check special prices in depot
    //                 const validSpecial = (
    //                   depotItem.pricelist_item_special_prices as any[]
    //                 ).find(sp => {
    ...
    //               const defaultItem = defaultPricelist.pricelist_item.find(
    //                 item => item.product_id === productId
    //               );
    //               if (defaultItem) {
    //                 // Check special prices in default
    //                 const validSpecial = (
    //                   defaultItem.pricelist_item_special_prices as any[]
    //                 ).find(sp => {
    //                   const from = sp.valid_from ? new Date(sp.valid_from) : null;
    //                   const to = sp.valid_to ? new Date(sp.valid_to) : null;
    //                   const afterFrom = !from || today_date >= from;
    ...
    //             const taxRate =
    //               priceEntry && priceEntry.taxRate !== null
    //                 ? priceEntry.taxRate
    //                 : Number(
    //                     (stock.inventory_stock_products as any)
    //                       ?.product_tax_master?.tax_rate
    //                   ) || 0;
    //             const convRate =
    //               Number(
    //                 (stock.inventory_stock_products as any)
    ...
    //             },
    //           });
    //           // Build loadQtyMap from van_inventory_items (loading records for this session).
    //           // Key: `{product_id}-{batch_number}` — matches the productMap keys built from inventory_stock.
    //           // For SAP-sourced loads where van_inventory_items stores the TOTAL quantity under
    //           // one primary batch_lot_id (e.g. KNMW 500ml with 329 Cases stored under CK6-019),
    //           // the map will have a single entry for that primary batch with the full load quantity.
    //           // The hasLoadEntry guard in the loop below ensures we trust this actual load record.
    ...
    //           }
    //           console.log(
    //             '[DEBUG Reconciliation] sessionStart:',
    //             sessionStart.toISOString()
    //           );
    //           console.log(
    //             '[DEBUG Reconciliation] todayEnd:',
    //             todayEnd.toISOString()
    //           );
    //           console.log(
    //             '[DEBUG Reconciliation] querying salesperson:',
    //             userIdNum
    //           );
    ...
    //             }
    //             const loadKey = `${p.product_id}-${p.batch_number || ''}`;
    //             const hasLoadEntry = loadQtyMap.has(loadKey);
        ...
        try {
            ...
            for (const vanLoc of vanLocations) {
                ...
                if (!locationId)
                    continue;
                try {
                    const reconciliationId = await prisma_client_1.default.$transaction(async (tx) => {
                        const stockToUnload = await tx.inventory_stock.findMany({
                            where: {
                                location_id: locationId,
                                salesperson_id: userIdNum,
                        ...
                        console.log('[DEBUG Reconciliation] sessionStart:', sessionStart.toISOString());
                        console.log('[DEBUG Reconciliation] todayEnd:', todayEnd.toISOString());
                        console.log('[DEBUG Reconciliation] querying salesperson:', userIdNum);
                        const saleInvoiceItems = await tx.invoice_items.findMany({
                            where: {
                                invoices: {
                                    OR: [{ salesperson_id: userIdNum }, { createdby: userIdNum }],
                        ...
                        const sessionInvoiceIds = Array.from(new Set(saleInvoiceItems
                        ...
                        console.log('[DEBUG Reconciliation] Found stock movements (sales by batch):', saleStockMovements.length);
                        ...
                        const productsWithExactBatchSale = new Set();
                        for (const mv of saleStockMovements) {
                            ...
                            console.log(`[DEBUG EXACT BATCH SALE] Product: ${mv.product_id}, Batch: ${batchNum}, Qty: ${mv.quantity}`);
                        }
                        ...
                        for (const record of saleInvoiceItems) {
                            ...
                            if (productsWithExactBatchSale.has(record.product_id))
                                continue;
                            const key = `${record.product_id}-`;
                            const current = saleQtyMap.get(key) || { qty: 0, baseQty: 0 };
                            saleQtyMap.set(key, {
                        ...
                        const toCreate = [];
                        const totalLoadBaseByProduct = new Map();
                        for (const p of productMap.values()) {
                            const loadKey = `${p.product_id}-${p.batch_number || ''}`;
                            ...
                            const effectiveLoadBaseUnits = p.convRate > 0
                                ? hasEntry
                                    ? load.qty * p.convRate + load.baseQty
                                ...
                                : hasEntry
                                    ? load.qty
                                    : p.total_qty;
                            totalLoadBaseByProduct.set(p.product_id, (totalLoadBaseByProduct.get(p.product_id) || 0) +
                                effectiveLoadBaseUnits);
                        }
                        const assignedSaleBaseByProduct = new Map();
                        const lastBatchKeyByProduct = new Map();
                        for (const [key, p] of productMap.entries()) {
                            lastBatchKeyByProduct.set(p.product_id, key);
                        }
                        for (const [mapKey, p] of productMap.entries()) {
                            ...
                            if (exactBatchSale) {
                                ...
                                console.log(`EXACT BATCH SALE Product: ${p.product_id}, Batch: ${p.batch_number}, saleQty: ${saleQty}`);
                            }
                            const totalSaleEntry = saleQtyMap.get(`${p.product_id}-`);
                            if (!exactBatchSale && totalSaleEntry) {
                                const totalSaleBaseUnits = p.convRate > 0
                                    ? totalSaleEntry.qty * p.convRate + totalSaleEntry.baseQty
                                    : totalSaleEntry.qty;
                                const totalLoadBaseUnits = totalLoadBaseByProduct.get(p.product_id) || 0;
                                const loadKey = `${p.product_id}-${p.batch_number || ''}`;
                                ...
                                const thisBatchLoadBaseUnits = p.convRate > 0
                                    ? hasBatchEntry
                                        ? thisBatchLoad.qty * p.convRate + thisBatchLoad.baseQty
                                        : p.total_qty * p.convRate + p.total_base_qty
                                    : hasBatchEntry
                                        ? thisBatchLoad.qty
                                        : p.total_qty;
                                const alreadyAssigned = assignedSaleBaseByProduct.get(p.product_id) || 0;
                                const isLastBatch = lastBatchKeyByProduct.get(p.product_id) === mapKey;
                                let batchSaleBaseUnits;
                                if (isLastBatch) {
                                    batchSaleBaseUnits = totalSaleBaseUnits - alreadyAssigned;
                                }
                                else if (totalLoadBaseUnits > 0) {
                                    batchSaleBaseUnits = Math.floor((thisBatchLoadBaseUnits / totalLoadBaseUnits) *
                                        totalSaleBaseUnits);
                                }
                                else {
                                    batchSaleBaseUnits =
                                ...
                                console.log(`DEBUG SALES DIST Product: ${p.product_id}, Batch: ${p.batch_number}, isLastBatch: ${isLastBatch}, totalLoadBaseUnits: ${totalLoadBaseUnits}, thisBatchLoadBaseUnits: ${thisBatchLoadBaseUnits}, totalSaleBaseUnits: ${totalSaleBaseUnits}, batchSaleBaseUnits: ${batchSaleBaseUnits}`);
                                batchSaleBaseUnits = Math.max(0, batchSaleBaseUnits);
                                assignedSaleBaseByProduct.set(p.product_id, alreadyAssigned + batchSaleBaseUnits);
                                if (p.convRate > 0) {
                                    saleQty = Math.floor(batchSaleBaseUnits / p.convRate);
                                    saleBaseQty = batchSaleBaseUnits % p.convRate;
                                }
                            ...
                            const saleVal = saleQty * p.price + saleBaseQty * unitPricePerPc;
                            const taxAmount = (saleVal * p.taxRate) / 100;
                            const calculatedExpectedBase = Math.max(0, loadQty * p.convRate +
                                loadBaseQty -
                                (saleQty * p.convRate + saleBaseQty));
                            const calculatedExpectedQty = p.convRate > 0
                                ? Math.floor(calculatedExpectedBase / p.convRate)
                                : Math.max(0, loadQty - saleQty);
                            const calculatedExpectedBaseQty = p.convRate > 0 ? calculatedExpectedBase % p.convRate : 0;
                            toCreate.push({
                                reconciliation_id: recon.id,
                                product_id: p.product_id,
                                batch_number: p.batch_number,
                    ...
                    console.error(`Failed to process location ${locationId}:`, vanLocError);
                    errors.push(`Location ${locationId}: ${vanLocError.message}`);
                }
            }
            return res.json({
                success: true,
                message: 'Stock staged for reconciliation. Save the reconciliation to submit for unload approval.',
                data: {
                    user_id: userIdNum,
                    reconciliation_ids: reconciliationIds,
                    total_items_requested: totalItemsRequested,
    ...
    async getProducts(req, res) {
        try {
            ...
            let vanInventory;
            if (depot_id) {
                vanInventory = await prisma_client_1.default.van_inventory.findFirst({
                    where: {
            ...
            const availableProducts = vanInventory.van_inventory_items_inventory.filter(item => {
