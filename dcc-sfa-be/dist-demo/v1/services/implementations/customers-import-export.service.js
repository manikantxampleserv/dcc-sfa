...
class CustomersImportExportService extends import_export_service_1.ImportExportService {
    ...
    columns = [
        ...
        },
        {
            ...
            type: 'number',
            transform: value => (value ? parseInt(value) : null),
            description: 'ID of the region this customer belongs to (optional) - see Ref - Regions sheet',
        },
        {
            key: 'district_id',
            header: 'District ID',
            ...
            type: 'number',
            transform: value => (value ? parseInt(value) : null),
            description: 'ID of the district this customer belongs to (optional) - see Ref - Districts sheet',
        },
        {
    ...
    async getSampleData() {
        const [zones, customerTypes, customerChannels, routes, users, regions, districts,] = await Promise.all([
            ...
            prisma_client_1.default.regions.findFirst({ select: { id: true } }),
            prisma_client_1.default.districts.findFirst({ select: { id: true } }),
        ]);
        return [
            {
    ...
    async validateForeignKeys(data, tx) {
        ...
        const cacheKey = (type, id) => `${type}_${id}`;
        const checkForeignKey = async (type, id, validator) => {
            if (!id)
                return null;
        ...
        const validations = [
            ...
            data.region_id &&
                checkForeignKey('Region', data.region_id, async () => !!(await prismaClient.regions.findUnique({
                    where: { id: data.region_id },
                }))),
            data.district_id &&
                checkForeignKey('District', data.district_id, async () => !!(await prismaClient.districts.findUnique({
    ...
    async updateExisting(data, userId, tx) {
        ...
        const updateData = {
            ...
            region_id: data.region_id !== undefined ? data.region_id : existing.region_id,
