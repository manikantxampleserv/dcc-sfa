...
exports.CustomerAssetsImportExportService = void 0;
const prisma_client_1 = __importDefault(require("../../../configs/prisma.client"));
const import_export_service_1 = require("../base/import-export.service");
class CustomerAssetsImportExportService extends import_export_service_1.ImportExportService {
    modelName = 'customer_assets';
    displayName = 'Customer Assets';
    uniqueFields = ['customer_id', 'serial_number'];
    searchFields = [
        'model',
        'serial_number',
        'status',
    ...
    columns = [
        ...
        },
        {
            ...
            validation: value => {
                ...
                const id = parseInt(value);
                if (isNaN(id) || id <= 0)
                    return 'Brand ID must be a positive number';
                return true;
            },
        ...
        },
        {
            ...
            description: 'Asset status: working, faulty, under_repair, replaced, retired, pending (defaults to working)',
        },
        {
        ...
        },
        {
            ...
            description: 'Maintenance contract number or details (optional, max 100 chars)',
        },
        {
            key: 'is_active',
            header: 'Is Active',
    ...
    async getSampleData() {
        ...
        const assetTypes = await prisma_client_1.default.asset_types.findMany({
            take: 3,
            select: { id: true, name: true },
            orderBy: { id: 'asc' },
        });
        const brands = await prisma_client_1.default.brands.findMany({
            take: 3,
            select: { id: true, name: true },
            orderBy: { id: 'asc' },
        });
    ...
    async checkDuplicate(data, tx) {
        const model = tx ? tx.customer_assets : prisma_client_1.default.customer_assets;
    ...
    async importData(data, userId, options = {}) {
        ...
        for (const [index, row] of data.entries()) {
            ...
            try {
                ...
                const created = await prisma_client_1.default.customer_assets.create({
                    data: preparedData,
                });
    ...
    async updateExisting(data, userId, tx) {
        ...
        const updateData = {
            ...
            last_scanned_date: data.last_scanned_date !== undefined
            ...
            maintenance_contract: data.maintenance_contract !== undefined
...
exports.CustomerAssetsImportExportService = CustomerAssetsImportExportService;
//# sourceMappingURL=customer-assets.import-export.service.js.map