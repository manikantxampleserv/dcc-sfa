...
exports.RouteImportExportService = void 0;
const import_export_service_1 = require("../base/import-export.service");
const prisma_client_1 = __importDefault(require("../../../configs/prisma.client"));
class RouteImportExportService extends import_export_service_1.ImportExportService {
    modelName = 'routes';
    displayName = 'Routes';
    uniqueFields = ['code'];
    ...
    masterTableConfigs = [
        {
            ...
            sheetName: 'Ref - Zones',
            description: 'Use the ID from this sheet in the Zone ID (Parent ID) column',
        },
        {
            masterTable: 'depots',
            masterKey: 'id',
    ...
    columns = [
        ...
        },
        {
            ...
            description: 'Route code (optional, auto-generated if empty, max 50 chars)',
        },
        {
            key: 'parent_id',
        ...
        },
        {
            ...
            validation: value => {
                if (!value)
                    return 'Route Type ID is required';
                const id = parseInt(value);
                if (isNaN(id) || id <= 0)
        ...
        },
        {
            ...
            required: false,
            type: 'string',
            validation: value => !value || value.length <= 500 || 'Max 500 characters',
        ...
        },
        {
            ...
            required: false,
            type: 'string',
            validation: value => !value || value.length <= 255 || 'Max 255 characters',
    ...
    async getSampleData() {
        ...
        const routeTypes = await prisma_client_1.default.route_type.findMany({
            take: 2,
            select: { id: true, name: true },
            where: { is_active: 'Y' },
        });
        ...
        if (!zoneId1 || !depotId1 || !routeTypeId1) {
            console.warn('⚠️ Missing required data for route sample. Cannot generate.');
            return [];
        }
        return [
            {
    ...
    async checkDuplicate(data, tx) {
        const model = tx ? tx.routes : prisma_client_1.default.routes;
        if (data.code) {
    ...
    async updateExisting(data, userId, tx) {
        ...
        const updateData = {
            ...
            route_type: data.route_type !== undefined ? data.route_type : existing.route_type,
            outlet_group: data.outlet_group !== undefined
                ? data.outlet_group
                : existing.outlet_group,
            start_location: data.start_location !== undefined
                ? data.start_location
                : existing.start_location,
            end_location: data.end_location !== undefined
                ? data.end_location
                : existing.end_location,
            estimated_distance: data.estimated_distance !== undefined
            ...
            route_type_id: data.route_type_id !== undefined
                ? data.route_type_id
    ...
    async exportToExcel(options = {}) {
        ...
        const data = await prisma_client_1.default.routes.findMany(query);
...
exports.RouteImportExportService = RouteImportExportService;
//# sourceMappingURL=route-import-export.service.js.map