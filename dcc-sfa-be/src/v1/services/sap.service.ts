              ...
              if (trackingType === 'BATCH') {
                ...
                for (const batchInput of batchData) {
                  ...
                  if (existingVanItem) {
                    ...
                    await tx.van_inventory_items.update({
                      ...
                      data: {
                        sap_docnum:
                          inventoryData.sap_docnum ||
                          existingVanItem.sap_docnum,
                        sap_docentry:
                          inventoryData.sap_docentry?.toString() ||
                          existingVanItem.sap_docentry,
                        source_system:
                          inventoryData.source_system ||
                    ...
                    await tx.van_inventory_items.create({
                      data: {
                        ...
                        sap_docentry:
                          inventoryData.sap_docentry?.toString() || null,
                ...
                if (existingVanItem) {
                  await tx.van_inventory_items.update({
                    ...
                    data: {
                      sap_docnum:
                        inventoryData.sap_docnum || existingVanItem.sap_docnum,
                      sap_docentry:
                        inventoryData.sap_docentry?.toString() ||
                        existingVanItem.sap_docentry,
                      source_system:
                  ...
                  await tx.van_inventory_items.create({
                    data: {
                      ...
                      source_system: inventoryData.source_system || null,
                      is_cancelled:
                        item.is_cancelled || inventoryData.is_cancelled || 'Y',
                      remarks: item.remarks || inventoryData.remarks || null,
                      tax_amount: Number(item.tax_amount || 0),
