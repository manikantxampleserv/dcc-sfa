...
export class VanInventoryImportExportService extends ImportExportService<any> {
  ...
  protected masterTableConfigs = [
    ...
    {
      masterTable: 'batch_lots' as any,
      masterKey: 'id',
      masterDisplayFields: ['id', 'batch_number', 'expiry_date', 'quantity'],
      sheetName: 'Ref - Batches',
      description:
        'Use the batch_number from this sheet in the Batch Number column',
    },
    {
      masterTable: 'serial_numbers' as any,
      masterKey: 'id',
      masterDisplayFields: ['id', 'serial_number', 'status'],
      sheetName: 'Ref - Serials',
      description:
        'Use the serial_number from this sheet in the Serial Numbers column',
    },
  ];
  protected columns: ColumnDefinition[] = [
    ...
    },
    {
      ...
      type: 'string',
      description:
        'Comma-separated serial numbers for serial-tracked products (optional)',
    },
  ];
  ...
  async importData(
    ...
    const groupedData = new Map<
      string,
      { header: any; items: any[]; rows: number[] }
    >();
    ...
    for (const [groupKey, group] of groupedData.entries()) {
      ...
      const rowErrors: any = { row: group.rows[0], errors: [] };
      try {
        await prisma.$transaction(
          async tx => {
            ...
            if (items && items.length > 0) {
              await processVanInventoryItems(
                tx,
                created,
                items,
                userId,
                preparedData.loading_type,
                preparedData
              );
            }
