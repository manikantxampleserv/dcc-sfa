    ...
    for (const fk of fkValidations) {
      if (data[fk.key]) {
        const error = await checkCache(fk.table, data[fk.key], async () => {
          const record = await (prismaClient as any)[fk.table].findUnique({
            where: { id: data[fk.key] },
          });
          if (!record) return `${fk.label} with ID "${data[fk.key]}" not found`;
          return null;
        });
...
- Use the system's master data to get the correct IDs for all related entities.
  ...
  protected async updateExisting(
    ...
    if (updateData.sap_code && updateData.sap_code !== existing.sap_code) {
      ...
      if (existingSapCode) {
        throw new Error(
          `Product SAP code "${updateData.sap_code}" already exists`
        );
      }
    }
