...
export class CoolerInspectionsImportExportService extends ImportExportService<any> {
  ...
  protected displayName = 'Cooler Inspections';
  protected uniqueFields = ['id'];
  protected searchFields = ['issues', 'action_taken'];
  ...
  protected async transformDataForExport(data: any[]): Promise<any[]> {
    return data.map(inspection => {
      const cooler = inspection.coolers as any;
      const customer = cooler?.coolers_customers as any;
      const depot = customer?.customer_depot as any;
      const coolerType = cooler?.cooler_types as any;
      const coolerSubType = cooler?.cooler_sub_types as any;
      const assetMaster = cooler?.cooler_asset_master as any;
      const inspector = inspection.users as any;
      ...
      return {
        ...
        type:
          coolerType?.name || assetMaster?.asset_master_asset_types?.name || '',
        sub_type:
          coolerSubType?.name ||
          coolerSubType?.code ||
  ...
  protected async validateForeignKeys(
    ...
    if (data.cooler_id) {
      try {
        ...
        if (!cooler) return `Cooler with ID ${data.cooler_id} does not exist`;
      } catch {
        return `Invalid Cooler ID ${data.cooler_id}`;
      }
    }
    ...
    if (data.inspected_by) {
      try {
        ...
        if (!user) return `User with ID ${data.inspected_by} does not exist`;
      } catch {
        return `Invalid Inspector ID ${data.inspected_by}`;
      }
    }
    ...
    if (data.visit_id) {
      try {
        ...
        if (!visit) return `Visit with ID ${data.visit_id} does not exist`;
      } catch {
        return `Invalid Visit ID ${data.visit_id}`;
      }
    }
  ...
  async exportToExcel(options: any = {}): Promise<Buffer> {
    ...
    if (lastScannedDateVal) {
      const targetDate = new Date(lastScannedDateVal as string);
