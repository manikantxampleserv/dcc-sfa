...
export class CoolerSubTypesImportExportService extends ImportExportService<any> {
  protected modelName = 'cooler_sub_types' as const;
  protected displayName = 'Cooler Sub Types';
  ...
  protected columns: ColumnDefinition[] = [
    ...
    },
    {
      ...
      description:
        'Unique code for the cooler sub type (optional, max 100 chars)',
    },
    {
      ...
      type: 'string',
      validation: value => {
        if (!value) return 'Cooler type name is required';
        return true;
      },
  ...
  protected async checkDuplicate(data: any, tx?: any): Promise<string | null> {
    const model = tx ? tx.cooler_sub_types : prisma.cooler_sub_types;
