...
export const reportsController = {
  ...
  async getSalesVsTargetReport(req: Request, res: Response) {
    try {
      ...
      const actualSales = await prisma.invoice_items.findMany({
        where: {
          invoices: {
            is_active: 'Y',
      ...
      const salespersonIdsToFetch = Array.from(actualSalespersonIds).filter(
        id => !salespersonMap.has(id) || salespersonMap.get(id)?.name === 'N/A'
      );
      ...
      for (const salespersonId of filteredSalespersonIds) {
        for (const target of salesTargets) {
          ...
          const isMember =
            target.sales_targets_groups?.sales_target_group_members_id?.some(
              (member: any) => member.sales_person_id === salespersonId
            );
          ...
          if (isMember) {
            ...
            if (targetAmount > 0) {
              ...
              achievement = Math.min(
                (sales.quantity / targetQuantity) * 100,
                100
              );
            }
      ...
      const totalTargetAmount = performanceData.reduce(
        (sum, data) => sum + data.target_amount,
        0
      );
      const totalActualSales = performanceData.reduce(
        (sum, data) => sum + data.actual_sales,
        0
      );
