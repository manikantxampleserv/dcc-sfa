...
export const vanInventoryController = {
  ...
  async getSalespersonInventoryItems(req: Request, res: Response) {
    try {
      ...
      const filteredItems = itemsRaw.filter((it: any) => {
        ...
        if (!it.serial_id && !it.batch_lot_id && stagedProductIds.has(it.product_id)) return false;
        return true;
      });
  ...
  async getSalespersonInventoryItemsDropdown(req: Request, res: Response) {
    try {
      ...
      const filteredItems = items.filter((it: any) => {
        if (it.serial_id && stagedSerials.has(it.serial_id)) return false;
        if (it.van_inventory_items_batch_lot?.id && stagedBatches.has(it.van_inventory_items_batch_lot.id)) return false;
        if (!it.serial_id && !it.van_inventory_items_batch_lot?.id && stagedProductIds.has(it.product_id)) return false;
        return true;
      });
  ...
  async getinventoryItemSalesperson(req: Request, res: Response) {
    try {
      ...
      const stagedStocksBySalesperson = new Map<number, {
      ...
      const processVanInventoryItems = (vanInventories: any[], currentSalespersonId: number) => {
        let totalQuantity = 0;
        let totalRemainingQuantity = 0;
        const allProducts = new Set<number>();
        let totalBatches = 0;
        ...
        const processedVanInventories = vanInventories
          .map(vanInventory => {
            ...
            const items = vanInventory.van_inventory_items_inventory.filter((it: any) => {
              if (stagedEntry) {
                if (it.serial_id && stagedEntry.stagedSerials.has(it.serial_id)) return false;
                if (it.batch_lot_id && stagedEntry.stagedBatches.has(it.batch_lot_id)) return false;
                if (!it.serial_id && !it.batch_lot_id && stagedEntry.stagedProductIds.has(it.product_id)) return false;
              }
              return true;
            });
