...
export async function resolveRequesterDepotId(
  tx: any,
  requesterId: number,
  requestType: string,
  requestDataStr?: string | null
): Promise<number | null> {
  ...
  if (requestDataStr) {
    try {
      ...
      if (potentialDepotId) {
        ...
        if (userDepots.some((ud: any) => ud.depot_id === depotIdNum)) {
          return depotIdNum;
        }
      }
    } catch (e) {
  ...
  try {
    ...
    if (data.request_data) {
      try {
        ...
        const consolidateBatches = (itemsArr: any[]) => {
          if (!Array.isArray(itemsArr)) return itemsArr;
        ...
        if (parsed.items) parsed.items = consolidateBatches(parsed.items);
        if (parsed.van_inventory_items)
          parsed.van_inventory_items = consolidateBatches(
            parsed.van_inventory_items
          );
    ...
    try {
      ...
      const result = await prisma.$transaction(
        async tx => {
          ...
          const resolvedDepotId = await resolveRequesterDepotId(
            tx,
              ...
              try {
                ...
                if (reconForStock?.salesman_id) {
                  ...
                  if (locationIds.length > 0) {
                    ...
                    console.log(
                      `[RECONCILIATION_APPROVAL] Zeroed inventory_stock for salesman ${reconForStock.salesman_id} after settlement approval.`
                    );
                  }
                }
              } catch (stockZeroErr) {
                console.error(
                  '[RECONCILIATION_APPROVAL] Error zeroing inventory_stock after approval:',
                  stockZeroErr
                );
              }
