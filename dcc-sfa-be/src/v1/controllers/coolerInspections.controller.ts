...
const serializeCoolerInspection = (
  ...
  images: inspection.images ? JSON.parse(inspection.images) : null,
    ...
    ? {
        ...
        asset_master: inspection.coolers.cooler_asset_master
          ? {
              ...
              serial_number:
                inspection.coolers.cooler_asset_master.serial_number,
            }
          : null,
...
export const coolerInspectionsController = {
  async createCoolerInspection(req: any, res: any) {
    try {
      const data = req.body;
  ...
  async updateCoolerInspection(req: any, res: any) {
    try {
      const { id } = req.params;
      ...
      if (data.cooler_id) updateData.cooler_id = parseInt(data.cooler_id);
      ...
      if (data.is_working) updateData.is_working = data.is_working;
  ...
  async deleteCoolerInspection(req: Request, res: Response) {
    try {
      ...
      if (existingInspection.images) {
        try {
          const imageUrls: string[] = JSON.parse(existingInspection.images);
