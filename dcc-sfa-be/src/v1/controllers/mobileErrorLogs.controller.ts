...
export const mobileErrorLogsController = {
  async getMobileErrorLogs(req: Request, res: Response) {
    try {
      ...
      if (!model) {
        return res.status(500).json({
          success: false,
          message:
            'Prisma client was not initialized with mobile_error_logs. Please restart the backend server.',
        });
      }
      ...
      const searchTerm = ((search || error_message) as string)?.trim();
      if (searchTerm) {
        where.OR = [
      ...
      if (error_type) {
        where.error_type = (error_type as string).trim();
      }

      if (screen_name) {
        where.screen_name = { contains: (screen_name as string).trim() };
      }

      if (device_info) {
        where.device_info = { contains: (device_info as string).trim() };
      }
      ...
      if (is_synced !== undefined) {
        where.is_synced = parseInt(is_synced as string, 10);
      }
  ...
  async syncMobileErrorLogs(req: Request, res: Response) {
    try {
      const model = (prisma as any).mobile_error_logs;
      if (!model) {
        return res.status(500).json({
      ...
      if (!rawLogs.length || !rawLogs[0]?.error_message) {
        ...
        return res.status(400).json({
          success: false,
          message:
            'Invalid payload. At least one error log with error_message is required.',
        });
      }

      const logsToInsert = rawLogs.map((item: any) => ({
        error_message: String(
          item.error_message || item.message || 'Unknown Error'
        ),
        ...
        is_synced: 1,
        createdate:
          item.createdate || item.created_at
