...
export const userController = {
  ...
  async updateUser(req: any, res: any): Promise<void> {
    try {
      ...
      if (depot_ids !== undefined) {
        ...
        const userRoutes = await prisma.route_salespersons.findMany({
          where: { user_id: targetUserId },
          include: {
            route: true,
          },
        ...
        const routesToRemove = userRoutes.filter(
          rs => rs.route && !parsedDepotIds.includes(rs.route.depot_id)
        );
  ...
  async deleteUser(req: any, res: any): Promise<void> {
    try {
      ...
      if (user._count.orders_salesperson_users > 0) {
        constraints.push(
          `${user._count.orders_salesperson_users} order(s) as salesperson`
        );
      }
      if (user._count.orders_approved_by > 0) {
        constraints.push(
          `${user._count.orders_approved_by} order(s) as approver`
        );
      }
      ...
      if (user._count.cooler_inspections > 0) {
        constraints.push(
          `${user._count.cooler_inspections} cooler inspection(s)`
        );
      }
      if (user._count.asset_movements_performed_by > 0) {
        constraints.push(
          `${user._count.asset_movements_performed_by} asset movement(s)`
        );
      }
      if (user._count.asset_maintenance_technician > 0) {
        constraints.push(
          `${user._count.asset_maintenance_technician} maintenance record(s)`
        );
      }
      if (user._count.payments_payments_collected_byTousers > 0) {
        constraints.push(
          `${user._count.payments_payments_collected_byTousers} payment(s)`
        );
      }
      if (user._count.return_requests_users > 0) {
        constraints.push(
          `${user._count.return_requests_users} return request(s)`
        );
      }
      if (user._count.delivery_schedules_users > 0) {
        constraints.push(
          `${user._count.delivery_schedules_users} delivery schedule(s)`
        );
      }
      ...
      if (error.code === 'P2003') {
        res.error(
          'Cannot delete user because it is referenced by other records. Please remove or reassign these records first.',
          400
        );
  ...
  async getUsersDropdown(req: any, res: any): Promise<void> {
    try {
      ...
      if (role_name) {
        ...
        const hasContainer = roles.some(
          r =>
            r.toLowerCase().includes('container') ||
  ...
  async updateUserLogInst(req: any, res: any): Promise<void> {
    try {
      ...
      if (
        logInstVal === undefined ||
        logInstVal === null ||
        isNaN(Number(logInstVal))
      ) {
        res.error(
          'log_inst is required in request body and must be a number',
          400
        );
        return;
      }
