...
export async function processVanInventoryItems(
  tx: any,
  inventory: any,
  items: any[],
  userId: number,
...
export async function validateAndGetLocationId(
  tx: any,
...
export function getOrderedQuantities(item: any): {
  orderedQty: number;
...
export async function getContainerGroupUsers(
  ...
  if (activeContainerAsMain) {
    const subUserIds = activeContainerAsMain.van_inventory_sub_users.map(
      (su: any) => su.user_id
    );
  ...
  if (activeContainerAsSub?.van_inventory) {
    ...
    const subUserIds =
      activeContainerAsSub.van_inventory.van_inventory_sub_users.map(
        (su: any) => su.user_id
      );
