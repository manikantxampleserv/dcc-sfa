...
const mockCustomerChannels: MockCustomerChannel[] = [
  {
...
export async function seedCustomerChannel(): Promise<void> {
  try {
    for (const customerChannel of mockCustomerChannels) {
      const existingChannel = await prisma.customer_channel.findFirst({
        where: { channel_name: customerChannel.channel_name },
      });

      if (!existingChannel) {
        await prisma.customer_channel.create({
          data: {
...
export async function clearCustomerChannel(): Promise<void> {
  try {
    await prisma.customer_channel.deleteMany({});
  } catch (error: any) {
    if (
      error?.code === 'P2003' ||
      ...
      console.warn(
        '⚠️  Could not clear all customer channels due to foreign key constraints. Some records may be in use by customers.'
      );
    } else {
      throw error;
    }
