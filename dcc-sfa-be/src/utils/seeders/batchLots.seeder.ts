...
const mockBatchLots: MockBatchLot[] = [
  {
 ...
 */
export async function seedBatchLots(): Promise<void> {
  try {
    for (const batchLot of mockBatchLots) {
      const existingBatchLot = await prisma.batch_lots.findFirst({
        where: {
      ...
      if (!existingBatchLot) {
        await prisma.batch_lots.create({
          data: {
            ...batchLot,
            createdate: new Date(),
            createdby: 1,
 ...
 */
export async function clearBatchLots(): Promise<void> {
  try {
    await prisma.batch_lots.deleteMany({});
  } catch (error) {
    throw error;
  }
}
