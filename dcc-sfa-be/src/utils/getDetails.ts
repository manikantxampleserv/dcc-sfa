...
async function getRequestDetailsByType(
  ...
  try {
    switch (request_type) {
      ...
      case 'COOLER_INSTALLATION_APPROVAL':
        ...
        return {
          ...
          customer_name: cooler.coolers_customers?.name || 'N/A',

          customer_code: cooler.coolers_customers?.code || 'N/A',

          technician_name: cooler.users?.name || 'N/A',

          asset_name: cooler.cooler_asset_master?.name || 'N/A',

          asset_code: cooler.cooler_asset_master?.code || 'N/A',

          asset_serial_number:
            cooler.cooler_asset_master?.serial_number || 'N/A',

          asset_type:
            cooler.cooler_asset_master?.asset_master_asset_types?.name || 'N/A',

          asset_sub_type:
            cooler.cooler_asset_master?.asset_master_asset_sub_types?.name ||
            'N/A',

          asset_brand:
            cooler.cooler_asset_master?.asset_master_brands?.name || 'N/A',

          current_asset_status:
            cooler.cooler_asset_master?.current_status || 'N/A',

          current_asset_location:
            cooler.cooler_asset_master?.current_location || 'N/A',

          cooler_type: cooler.cooler_types?.name || 'N/A',

          cooler_sub_type: cooler.cooler_sub_types?.name || 'N/A',

          install_date: cooler.install_date
            ? new Date(cooler.install_date).toLocaleDateString()
            : 'N/A',

          status: cooler.status || 'N/A',

          approval_status: cooler.approval_status || 'P',

          capacity: cooler.capacity || null,

          temperature: cooler.temperature ? Number(cooler.temperature) : null,

          energy_rating: cooler.energy_rating || 'N/A',

          maintenance_contract: cooler.maintenance_contract || 'N/A',
        };
      default:
        return {};
    }
