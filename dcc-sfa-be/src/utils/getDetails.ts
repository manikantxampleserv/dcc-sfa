      ...
      case 'RECONCILIATION_APPROVAL':
        ...
        return {
          ...
          items: reconciliation.reconciliation_items.map((item: any) => {
            ...
            const rawUom =
              item.product?.product_unit_of_measurement?.name || 'Cs';
            const rawSubUnit =
              item.product?.product_unit_of_measurement?.sub_unit || 'Btls';
            const uom =
              rawUom.toLowerCase().includes('case') ||
            ...
            return {
              ...
              sub_category_name:
                item.product?.product_sub_categories_products
                  ?.sub_category_name || '',
              uom,
              sub_unit: subUnit,
