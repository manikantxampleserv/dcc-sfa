...
exports.requireAllPermission = exports.requirePermission = exports.authenticateApiToken = exports.authenticateToken = void 0;
...
const authenticateToken = async (req, res, next) => {
    ...
    try {
        ...
        const dbToken = await prisma_client_1.default.api_tokens.findFirst({
            where: {
                token: token,
                user_id: decoded.id,
            },
...
const authenticateApiToken = async (req, res, next) => {
    try {
        ...
        const apiToken = await prisma_client_1.default.api_tokens.findFirst({
            where: {
                token: token,
            },
            include: {
...
const requirePermission = (permissions) => {
    return (req, res, next) => {
        ...
        const hasRequiredPermission = (0, permissions_config_1.hasAnyModulePermissions)(req.user.permissions, permissions);
        if (!hasRequiredPermission) {
            const requiredPermissions = permissions.map(p => (0, permissions_config_1.buildPermissionName)(p.module, p.action));
            const userFriendlyMessage = (0, permissions_config_1.formatPermissionErrorMessage)(permissions);
 ...
 * @returns {Function} Express middleware function
 */
const requireAllPermission = (permissions) => {
    return (req, res, next) => {
        if (!req.user) {
            return res.status(401).json({ error: 'authentication_required' });
        }
        ...
        const hasAllRequired = (0, permissions_config_1.hasAllModulePermissions)(req.user.permissions, permissions);
        if (!hasAllRequired) {
            const requiredPermissions = permissions.map(p => (0, permissions_config_1.buildPermissionName)(p.module, p.action));
            const userFriendlyMessage = (0, permissions_config_1.formatPermissionErrorMessage)(permissions);
            return res.status(403).json({
...
exports.requireAllPermission = requireAllPermission;
