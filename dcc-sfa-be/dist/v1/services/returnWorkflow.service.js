...
class ReturnWorkflowService {
    ...
    static async getWorkflowSteps(requestId) {
        try {
            const steps = await prisma_client_1.default.return_workflow.findMany({
                where: {
     ...
     */
    static async createInitialWorkflow(requestId, userId) {
        try {
            const initialSteps = [
                {
                    request_type: 'return',
            ...
            await prisma_client_1.default.return_workflow.createMany({
                data: initialSteps,
            });
        }
        catch (error) {
     ...
     */
    static async updateWorkflowStep(stepId, status, remarks, actionBy) {
        try {
            const updatedStep = await prisma_client_1.default.return_workflow.update({
                where: { id: stepId },
                data: {
                    status,
                    remarks,
     ...
     */
    static async addWorkflowStep(requestId, step, status, remarks, actionBy, userId) {
        try {
            const newStep = await prisma_client_1.default.return_workflow.create({
                data: {
                    request_type: 'return',
                    request_id: requestId,
                    step,
     ...
     */
    static async getWorkflowStepByStep(requestId, step) {
        try {
            const workflowStep = await prisma_client_1.default.return_workflow.findFirst({
                where: {
                    request_id: requestId,
                    request_type: 'return',
                    step,
     ...
     */
    static async executeFullWorkflowFlow(requestId, userId, templateId = 'standard_return') {
        try {
            ...
            for (let i = 0; i < template.length; i++) {
                ...
                const newStep = await this.addWorkflowStep(requestId, stepData.step, stepData.status, stepData.remarks, assignedUserId, userId);
            ...
            await prisma_client_1.default.return_requests.update({
                where: { id: requestId },
                data: {
                    status: 'completed',
                    approved_by: userId,
     ...
     */
    static async rejectReturnRequest(requestId, userId, rejectionReason) {
        try {
            // Get current workflow steps
            const currentSteps = await this.getWorkflowSteps(requestId);
            // Find the current active step
            const activeStep = currentSteps.find(step => step.status === 'in_progress');
            if (activeStep) {
                // Update the current step to rejected
                await this.updateWorkflowStep(activeStep.id, 'rejected', `Rejected: ${rejectionReason}`, userId);
            }
            // Create a rejection step
            const rejectionStep = await this.addWorkflowStep(requestId, 'Request Rejected', 'rejected', `Return request rejected: ${rejectionReason}`, userId, userId);
     ...
     */
    static async executeNextWorkflowStep(requestId, userId) {
        try {
            // Get current workflow steps
            const currentSteps = await this.getWorkflowSteps(requestId);
            ...
            const updatedStep = await this.updateWorkflowStep(nextStep.id, 'completed', `Step completed automatically by system`, userId);
            // Check if this was the last step
            const remainingSteps = currentSteps.filter(step => step.status === 'pending');
...
//# sourceMappingURL=returnWorkflow.service.js.map