...
const import_export_error_service_1 = require("./import-export-error.service");
const prisma_client_1 = __importDefault(require("../../../configs/prisma.client"));
class ImportExportService {
    getModel() {
        return prisma_client_1.default[this.modelName];
    }
    ...
    async parseExcelFile(buffer) {
        ...
        try {
            ...
            if (missingHeaders.length > 0) {
                errorHandler.addHeaderError(`Missing required columns: ${missingHeaders.join(', ')}`, { missingHeaders, foundHeaders: fileHeaders });
            }
            const expectedHeaders = this.columns.map(col => col.header);
            const unexpectedHeaders = fileHeaders.filter(header => !expectedHeaders.includes(header));
            if (unexpectedHeaders.length > 0) {
                console.warn(`Unexpected columns found and will be ignored: ${unexpectedHeaders.join(', ')}`);
            }
            jsonData.forEach((row, index) => {
                ...
                this.columns.forEach(column => {
                    ...
                    if (value !== undefined && value !== null && value !== '') {
                        try {
                            processedRow[column.key] = errorHandler.transformField(value, column, rowNum);
                        }
                        catch (error) {
                            rowHasError = true;
                        }
            ...
            errorHandler.addFileError(`Failed to parse Excel file: ${error.message}`, { originalError: error.toString() });
            return {
                data: [],
                errors: errorHandler.buildErrorResponse(),
                validCount: 0,
    ...
    async generateTemplate() {
        ...
        // Add master table reference sheets
        for (const config of this.masterTableConfigs) {
            ...
            if (masterData.length > 0) {
                ...
                // Create columns for master table
        ...
        // Add master table reference info
    ...
    async importData(data, userId, options = {}) {
        ...
        const results = await prisma_client_1.default.$transaction(async (tx) => {
            for (const [index, row] of data.entries()) {
                const rowNum = index + 2;
                const rowErrors = { row: rowNum, errors: [] };
                try {
    ...
    async getMasterTableData(config) {
        ...
        try {
            return await prisma_client_1.default[masterTable].findMany({
                select: masterDisplayFields.reduce((acc, field) => ({
                    ...acc,
                    [field]: true,
                }), {}),
                where: { is_active: 'Y' },
            ...
            console.warn(`Could not fetch master data for ${masterTable}:`, error);
            return [];
        }
    }
}
