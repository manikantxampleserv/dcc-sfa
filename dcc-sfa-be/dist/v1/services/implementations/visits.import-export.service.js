...
exports.VisitsImportExportService = void 0;
...
class VisitsImportExportService extends import_export_service_1.ImportExportService {
    ...
    columns = [
        ...
        },
        {
            ...
            validation: value => {
                ...
                const id = parseInt(value);
                if (isNaN(id) || id <= 0)
                    return 'Route ID must be a positive number';
                return true;
            },
        ...
        },
        {
            ...
            validation: value => {
                ...
                return (validPurposes.includes(value.toLowerCase()) ||
                    `Purpose must be one of: ${validPurposes.join(', ')}`);
            },
            transform: value => (value ? value.toLowerCase() : 'sales'),
            description: 'Visit purpose: sales, collection, delivery, survey, support, followup (defaults to sales)',
        },
        {
            key: 'status',
            header: 'Status',
            ...
            description: 'Visit status: planned, in-progress, completed, cancelled, missed (defaults to planned)',
        },
        {
            key: 'start_time',
            header: 'Start Time',
        ...
        },
        {
            ...
            validation: value => {
                ...
                if (isNaN(lat))
                    return 'Start latitude must be a number';
                if (lat < -90 || lat > 90)
        ...
        },
        {
            ...
            validation: value => {
                ...
                if (isNaN(lng))
                    return 'Start longitude must be a number';
                if (lng < -180 || lng > 180)
        ...
        },
        {
            ...
            validation: value => {
                ...
                const lat = parseFloat(value);
                if (isNaN(lat))
                    return 'End latitude must be a number';
                if (lat < -90 || lat > 90)
        ...
        },
        {
            ...
            validation: value => {
                ...
                const lng = parseFloat(value);
                if (isNaN(lng))
                    return 'End longitude must be a number';
                if (lng < -180 || lng > 180)
        ...
        },
        {
            ...
            transform: value => value !== null && value !== undefined ? parseInt(value) : 0,
            description: 'Number of orders created during visit (optional, defaults to 0)',
        },
        {
            ...
            validation: value => {
                ...
                if (isNaN(amount))
                    return 'Amount collected must be a number';
                if (amount < 0)
                    return 'Amount collected cannot be negative';
    ...
    async prepareDataForImport(data, userId) {
        const preparedData = {
            ...
            orders_created: data.orders_created !== undefined ? data.orders_created : 0,
        ...
        if (data.start_longitude !== null && data.start_longitude !== undefined) {
            preparedData.start_longitude = new client_1.Prisma.Decimal(data.start_longitude);
        }
        if (data.end_latitude !== null && data.end_latitude !== undefined) {
            preparedData.end_latitude = new client_1.Prisma.Decimal(data.end_latitude);
        }
        if (data.end_longitude !== null && data.end_longitude !== undefined) {
            preparedData.end_longitude = new client_1.Prisma.Decimal(data.end_longitude);
        }
        if (data.amount_collected !== null && data.amount_collected !== undefined) {
            preparedData.amount_collected = new client_1.Prisma.Decimal(data.amount_collected);
        }
        else {
    ...
    async updateExisting(data, userId, tx) {
        ...
        const updateData = {
            ...
            visit_time: data.visit_time !== undefined ? data.visit_time : existing.visit_time,
            ...
            orders_created: data.orders_created !== undefined
                ? data.orders_created
                : existing.orders_created,
            visit_notes: data.visit_notes !== undefined
                ? data.visit_notes
        ...
        if (data.start_latitude !== null && data.start_latitude !== undefined) {
            updateData.start_latitude = new client_1.Prisma.Decimal(data.start_latitude);
        }
        if (data.start_longitude !== null && data.start_longitude !== undefined) {
            updateData.start_longitude = new client_1.Prisma.Decimal(data.start_longitude);
        }
        if (data.end_latitude !== null && data.end_latitude !== undefined) {
            updateData.end_latitude = new client_1.Prisma.Decimal(data.end_latitude);
        }
        if (data.end_longitude !== null && data.end_longitude !== undefined) {
            updateData.end_longitude = new client_1.Prisma.Decimal(data.end_longitude);
        }
        if (data.amount_collected !== null && data.amount_collected !== undefined) {
            updateData.amount_collected = new client_1.Prisma.Decimal(data.amount_collected);
        }
    ...
    async updateExistingDirect(data, userId) {
        const visitDate = new Date(data.visit_date);
        const startOfDay = new Date(visitDate.setHours(0, 0, 0, 0));
        const endOfDay = new Date(visitDate.setHours(23, 59, 59, 999));
        const existing = await prisma_client_1.default.visits.findFirst({
            where: {
                customer_id: data.customer_id,
                sales_person_id: data.sales_person_id,
                visit_date: {
        ...
        return await prisma_client_1.default.visits.update({
            where: { id: existing.id },
            data: updateData,
        });
    }
    async exportToExcel(options = {}) {
        ...
        summarySheet.addRow({
            ...
            value: totalVisits > 0 ? (totalOrdersCreated / totalVisits).toFixed(2) : '0',
        });
        summarySheet.addRow({
            metric: 'Average Collection Per Visit',
            value: totalVisits > 0 ? (totalAmountCollected / totalVisits).toFixed(2) : '0',
        });
        summarySheet.addRow({ metric: '', value: '' });
...
exports.VisitsImportExportService = VisitsImportExportService;
//# sourceMappingURL=visits.import-export.service.js.map