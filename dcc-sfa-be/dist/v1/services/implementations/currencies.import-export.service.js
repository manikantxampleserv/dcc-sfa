...
class CurrenciesImportExportService extends import_export_service_1.ImportExportService {
    ...
    columns = [
        ...
        },
        {
            ...
            validation: value => {
                ...
                if (isNaN(rate))
                    return 'Exchange rate must be a number';
                if (rate <= 0)
            ...
            description: 'Exchange rate relative to base currency (optional, positive number)',
        },
        {
            key: 'is_base',
            ...
            description: 'Is this the base currency - Y for Yes, N for No (defaults to N)',
        },
        {
            key: 'is_active',
            header: 'Is Active',
    ...
    async prepareDataForImport(data, userId) {
        ...
        if (data.exchange_rate_to_base !== null &&
            data.exchange_rate_to_base !== undefined) {
            preparedData.exchange_rate_to_base = new client_1.Prisma.Decimal(data.exchange_rate_to_base);
        }
        return preparedData;
    }
    ...
    async updateExisting(data, userId, tx) {
        ...
        if (data.exchange_rate_to_base !== null &&
            data.exchange_rate_to_base !== undefined) {
            updateData.exchange_rate_to_base = new client_1.Prisma.Decimal(data.exchange_rate_to_base);
        }
    ...
    async exportToExcel(options = {}) {
        ...
        data.forEach((currency) => {
            const totalUsage = (currency._count?.credit_notes || 0) +
