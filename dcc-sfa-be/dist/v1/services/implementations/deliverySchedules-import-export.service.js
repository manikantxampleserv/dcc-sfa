...
exports.DeliverySchedulesImportExportService = void 0;
const import_export_service_1 = require("../base/import-export.service");
const ExcelJS = __importStar(require("exceljs"));
const prisma_client_1 = __importDefault(require("../../../configs/prisma.client"));
class DeliverySchedulesImportExportService extends import_export_service_1.ImportExportService {
    modelName = 'delivery_schedules';
    displayName = 'Delivery Schedules';
    uniqueFields = ['order_id', 'customer_id', 'scheduled_date'];
    searchFields = [
        'scheduled_time_slot',
    ...
    columns = [
        ...
        },
        {
            ...
            description: 'Delivery status (scheduled, in_transit, delivered, failed, cancelled, rescheduled, returned, refunded)',
        },
        {
            key: 'priority',
            header: 'Priority',
        ...
        return [
            {
                ...
                status: 'scheduled',
                priority: 'high',
                delivery_instructions: 'Deliver to main entrance, call customer 30 minutes before arrival',
            ...
            },
            {
                ...
                status: 'delivered',
                priority: 'medium',
                delivery_instructions: 'Leave package at reception if customer not available',
...
- Status and Priority have predefined values
    ...
    async transformDataForExport(data) {
        return data.map(schedule => ({
            ...
            rescheduled_date: schedule.rescheduled_date?.toISOString().split('T')[0] || '',
    ...
    async checkDuplicate(data, tx) {
        const model = tx ? tx.delivery_schedules : prisma_client_1.default.delivery_schedules;
    ...
    async validateForeignKeys(data) {
        ...
        const customer = await prisma_client_1.default.customers.findFirst({
        ...
        if (data.assigned_driver_id) {
            const driver = await prisma_client_1.default.users.findFirst({
    ...
    async exportToExcel(options = {}) {
        ...
        exportData.forEach((row, index) => {
            const excelRow = worksheet.addRow({
                ...row,
                order_number: data[index]?.delivery_schedules_customers_orders?.order_number || '',
                customer_name: data[index]?.delivery_schedules_customers?.name || '',
                vehicle_number: data[index]?.delivery_schedules_vehicles?.vehicle_number || '',
...
exports.DeliverySchedulesImportExportService = DeliverySchedulesImportExportService;
//# sourceMappingURL=deliverySchedules-import-export.service.js.map