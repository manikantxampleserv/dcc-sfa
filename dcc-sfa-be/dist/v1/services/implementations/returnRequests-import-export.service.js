...
exports.ReturnRequestsImportExportService = void 0;
const import_export_service_1 = require("../base/import-export.service");
const ExcelJS = __importStar(require("exceljs"));
const prisma_client_1 = __importDefault(require("../../../configs/prisma.client"));
class ReturnRequestsImportExportService extends import_export_service_1.ImportExportService {
    modelName = 'return_requests';
    displayName = 'Return Requests';
    uniqueFields = ['customer_id', 'product_id', 'serial_id'];
    searchFields = ['reason', 'resolution_notes', 'status'];
    columns = [
        {
            key: 'customer_id',
            header: 'Customer ID',
        ...
        },
        {
            ...
            description: 'Return request status (pending, approved, rejected, processing, completed, cancelled)',
        },
        {
            key: 'approved_by',
        ...
        },
        {
            ...
            validation: value => {
                if (value) {
                    ...
                    today.setHours(23, 59, 59, 999);
                    if (date > today)
                        return 'Approved date cannot be in the future';
                }
                return true;
            },
            transform: value => (value ? new Date(value) : null),
...
- Status has predefined values
    ...
    async checkDuplicate(data, tx) {
        const model = tx ? tx.return_requests : prisma_client_1.default.return_requests;
    ...
    async validateForeignKeys(data) {
        ...
        if (data.serial_id) {
            const serialNumber = await prisma_client_1.default.serial_numbers.findFirst({
        ...
        if (data.approved_by) {
            const approver = await prisma_client_1.default.users.findFirst({
    ...
    async exportToExcel(options = {}) {
        ...
        const normalizeFilters = (filters) => {
            ...
            Object.keys(filters).forEach(key => {
                ...
                if (value &&
                    ...
                    normalized[snakeKey] = value.map(item => typeof item === 'object' && item !== null
        ...
        exportData.forEach((row, index) => {
            const excelRow = worksheet.addRow({
                ...
                serial_number: data[index]?.return_requests_serial_numbers?.serial_no || '',
...
exports.ReturnRequestsImportExportService = ReturnRequestsImportExportService;
//# sourceMappingURL=returnRequests-import-export.service.js.map