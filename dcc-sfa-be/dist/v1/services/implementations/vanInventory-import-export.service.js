    ...
    columns = [
        ...
        },
        {
            ...
            validation: value => {
                ...
                return (['D', 'A', 'C'].includes(upperValue) ||
                    'Must be D (Draft), A (Confirmed), or C (Cancelled)');
            },
            transform: value => (value ? value.toString().toUpperCase() : 'A'),
            description: 'Status - D = Draft, A = Confirmed, C = Cancelled (defaults to A)',
        },
        {
            key: 'loading_type',
            header: 'Loading Type',
        ...
        },
        {
            ...
            validation: value => {
                ...
                return (validTypes.includes(value.toLowerCase()) || 'Invalid location type');
            },
        ...
        // Van Inventory Items columns (for nested import/export)
        {
            ...
            description: 'JSON array of items: [{"product_id": 1, "quantity": 10, "batch_id": null, "serial_no_id": null, ...}]',
        },
    ];
    async getSampleData() {
        ...
        const batches = await prisma_client_1.default.batch_lots.findMany({
            take: 3,
            select: { id: true, batch_number: true },
            orderBy: { id: 'asc' },
        });
        const serials = await prisma_client_1.default.serial_numbers.findMany({
            take: 3,
