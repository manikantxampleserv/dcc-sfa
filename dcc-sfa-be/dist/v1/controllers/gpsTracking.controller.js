...
exports.gpsTrackingController = {
    ...
    async createGPSLog(req, res) {
        try {
            ...
            const { latitude, longitude, accuracy_meters, speed_kph, battery_level, network_type, log_time, } = req.body;
            const gpsLog = await prisma_client_1.default.gps_logs.create({
                data: {
                    user_id: userId,
     ...
     */
    async getGPSTrackingData(req, res) {
        try {
            ...
            if (start_date || end_date) {
                where.log_time = {};
                if (start_date) {
                    where.log_time.gte = new Date(start_date);
                }
                if (end_date) {
                    where.log_time.lte = new Date(end_date);
                }
            }
            ...
            const userSummary = Array.from(new Set(gpsLogs.map(log => log.user_id))).map(userId => {
                const userLogs = gpsLogs.filter(log => log.user_id === userId);
                const avgSpeed = userLogs.reduce((sum, log) => sum + Number(log.speed_kph || 0), 0) /
                    userLogs.length;
                return {
                    user_id: userId,
    ...
    async getRealTimeGPSTracking(req, res) {
        try {
            ...
            const realTimeData = await Promise.all(users.map(async (user) => {
                const latestLog = await prisma_client_1.default.gps_logs.findFirst({
                    where: {
                        user_id: user.id,
                        is_active: 'Y',
                    },
            ...
            res.status(500).json({
                success: false,
                message: error.message || 'Failed to retrieve real-time GPS tracking data',
            });
        }
    },
    /**
    ...
    async getUserGPSPath(req, res) {
        try {
            ...
            const gpsPath = await prisma_client_1.default.gps_logs.findMany({
                where,
                include: {
                    users_gps_logs_user_idTousers: {
                        select: {
     ...
     */
    async getRouteEffectiveness(req, res) {
        try {
            const { start_date, end_date, salesperson_id, route_id, depot_id } = req.query;
            // Build date filter
            const dateFilter = {};
            if (start_date) {
            ...
            if (salesperson_id) {
                whereGpsLogs.user_id = parseInt(salesperson_id);
            }
            const gpsLogs = await prisma_client_1.default.gps_logs.findMany({
                where: whereGpsLogs,
                include: {
            ...
            const routeAnalysis = routes.map(route => {
                ...
                const routeCustomers = allRouteCustomers.filter(c => c.route_id === route.id);
                ...
                const completionRate = routeCustomers.length > 0
                    ...
                    : 0;
                const visitsWithDuration = routeVisits.filter(v => v.start_time && v.end_time);
                const avgDuration = visitsWithDuration.length > 0
                    ? visitsWithDuration.reduce((sum, v) => {
                        const duration = (new Date(v.end_time).getTime() -
                ...
                const distanceEfficiency = plannedDistance > 0
                ...
                return {
                    ...
                    in_progress_visits: routeVisits.filter(v => v.status === 'in_progress').length,
                    ...
                    efficiency_score: ((completionRate + Number(distanceEfficiency)) /
                        2).toFixed(2),
            ...
            const totalActualVisits = routeAnalysis.reduce((sum, r) => sum + r.actual_visits, 0);
            const totalCompletedVisits = routeAnalysis.reduce((sum, r) => sum + r.completed_visits, 0);
            const avgCompletionRate = routeAnalysis.length > 0
                ? routeAnalysis.reduce((sum, r) => sum + Number(r.completion_rate), 0) / routeAnalysis.length
                : 0;
            const avgEfficiencyScore = routeAnalysis.length > 0
                ? routeAnalysis.reduce((sum, r) => sum + Number(r.efficiency_score), 0) / routeAnalysis.length
                : 0;
            const summary = {
...
//# sourceMappingURL=gpsTracking.controller.js.map