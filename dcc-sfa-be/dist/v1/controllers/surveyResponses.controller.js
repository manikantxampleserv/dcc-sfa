...
const serializeSurveyResponse = (item) => ({
    id: item.id,
    ...
    customer: item.survey_response_customer
        ...
        : null,
    answers: item.survey_answer_responses?.map((ans) => ({
        id: ans.id,
...
exports.surveyResponseController = {
    async createOrUpdateSurveyResponse(req, res) {
        const data = req.body;
        ...
        try {
            ...
            if (isBulk) {
                return await exports.surveyResponseController.createBulkSurveyResponses(req, res);
            }
            let answerItems = [];
            ...
            if (responseId) {
                const existingResponse = await prisma_client_1.default.survey_responses.findUnique({
            ...
            if (req.file) {
                ...
                photoPath = await (0, blackbaze_1.uploadFile)(req.file.buffer, fileName, req.file.mimetype);
                if (existingPhotoPath && responseId) {
                    await (0, blackbaze_1.deleteFile)(existingPhotoPath);
                }
            }
            ...
            const result = await prisma_client_1.default.$transaction(async (tx) => {
                ...
                if (Array.isArray(answerItems) && answerItems.length > 0) {
                    for (const ans of answerItems) {
                        if (!ans.field_id) {
                            throw new Error('Invalid field_id in answers: missing field_id');
                        }
                ...
                const payload = {
                    ...
                    submitted_at: data.submitted_at && data.submitted_at.trim() !== ''
            ...
            if (req.file && photoPath) {
                try {
                    await (0, blackbaze_1.deleteFile)(photoPath);
                }
                catch (deleteError) {
                    console.error('Error deleting file after failed transaction:', deleteError);
                }
            }
    ...
    async createBulkSurveyResponses(req, res) {
                ...
                for (let i = 0; i < responses.length; i++) {
                    ...
                    try {
                        ...
                        if (responseData.customer_id) {
                            ...
                            if (!customerExists) {
                                throw new Error(`Response ${i + 1}: Invalid customer_id: ${responseData.customer_id}`);
                            }
                        }
                        let answerItems = [];
    ...
    async getAllSurveyResponses(req, res) {
        try {
            const { page, limit, search, status, survey_id, submitted_by, customer_id, } = req.query;
            ...
            const filters = {
                ...
                ...(survey_id && { parent_id: parseInt(survey_id, 10) }),
                ...(submitted_by && {
                    submitted_by: parseInt(submitted_by, 10),
                }),
                ...(customer_id && {
            ...
            const { data, pagination } = await (0, paginate_1.paginate)({
                model: prisma_client_1.default.survey_responses,
                filters,
                page: pageNum,
                limit: limitNum,
            ...
            const [totalResponses, activeResponses, inactiveResponses, responsesThisMonth,] = await Promise.all([
                prisma_client_1.default.survey_responses.count(),
                prisma_client_1.default.survey_responses.count({ where: { is_active: 'Y' } }),
                prisma_client_1.default.survey_responses.count({ where: { is_active: 'N' } }),
                prisma_client_1.default.survey_responses.count({
                    where: {
                        createdate: {
            ...
            const serializedData = data.map((r) => serializeSurveyResponse(r));
            res.success('Survey responses fetched successfully', serializedData, 200, pagination, {
    ...
    async getSurveyResponseById(req, res) {
        try {
            const { id } = req.params;
            const record = await prisma_client_1.default.survey_responses.findUnique({
                where: { id: Number(id) },
                include: {
                    surveys: true,
                    survey_responses_submitted_by_users: true,
    ...
    async deleteSurveyResponse(req, res) {
        try {
            const { id } = req.params;
            const existing = await prisma_client_1.default.survey_responses.findUnique({
                where: { id: Number(id) },
            });
            if (!existing)
                return res.status(404).json({ message: 'Survey response not found' });
            if (existing.photo_url) {
                try {
                    await (0, blackbaze_1.deleteFile)(existing.photo_url);
                }
                catch (error) {
                    console.error('Error deleting photo from Backblaze:', error);
                }
            }
            await prisma_client_1.default.survey_answers.deleteMany({
                where: { parent_id: Number(id) },
            });
            await prisma_client_1.default.survey_responses.delete({ where: { id: Number(id) } });
    ...
    async getSurveyAnswers(req, res) {
        try {
            const { responseId } = req.params;
            const answers = await prisma_client_1.default.survey_answers.findMany({
...
//# sourceMappingURL=surveyResponses.controller.js.map