    ...
    async takeActionOnRequest(req, res) {
        ...
        try {
            ...
            if (result.status === 'fully_approved' && 'request' in result) {
                ...
                console.log('Debug: Recipient email:', result.request.sfa_d_requests_requester?.email);
                if (!result.request.sfa_d_requests_requester?.email) {
                    console.error('Error: Requester email is missing');
                    return res.status(400).json({ message: 'Requester email not found' });
                }
                await (0, mailer_1.sendEmail)({
                    to: result.request.sfa_d_requests_requester.email,
                    subject: template.subject,
