...
exports.coolerSubTypesController = void 0;
const paginate_1 = require("../../utils/paginate");
const prisma_client_1 = __importDefault(require("../../configs/prisma.client"));
const serializeCoolerSubType = (coolerSubType) => ({
    id: coolerSubType.id,
...
exports.coolerSubTypesController = {
    async createCoolerSubType(req, res) {
        try {
            const data = req.body;
            if (!data.name) {
                return res
            ...
            const coolerSubType = await prisma_client_1.default.cooler_sub_types.create({
                data: {
                    ...data,
    ...
    async getCoolerSubTypes(req, res) {
        try {
            const { page = '1', limit = '10', search = '', isActive, coolerTypeId, } = req.query;
            ...
            const totalCoolerSubTypes = await prisma_client_1.default.cooler_sub_types.count();
            const activeCoolerSubTypes = await prisma_client_1.default.cooler_sub_types.count({
                where: { is_active: 'Y' },
            });
            const inactiveCoolerSubTypes = await prisma_client_1.default.cooler_sub_types.count({
                where: { is_active: 'N' },
            });
            const now = new Date();
            const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 1);
            const endOfMonth = new Date(now.getFullYear(), now.getMonth() + 1, 1);
            const newCoolerSubTypesThisMonth = await prisma_client_1.default.cooler_sub_types.count({
                where: {
                    createdate: {
                        gte: startOfMonth,
                        lt: endOfMonth,
            ...
            const { data, pagination } = await (0, paginate_1.paginate)({
                model: prisma_client_1.default.cooler_sub_types,
                filters,
                page: page_num,
                limit: limit_num,
            ...
            res.json({
                ...
                data: data.map((d) => serializeCoolerSubType(d)),
                meta: {
    ...
    async getCoolerSubTypeById(req, res) {
        try {
            const { id } = req.params;
            const coolerSubType = await prisma_client_1.default.cooler_sub_types.findUnique({
                where: { id: Number(id) },
                include: {
                    cooler_sub_types_cooler_types: {
                        select: {
    ...
    async updateCoolerSubType(req, res) {
        try {
            const { id } = req.params;
            const existingCoolerSubType = await prisma_client_1.default.cooler_sub_types.findUnique({
                where: { id: Number(id) },
            });
            ...
            const coolerSubType = await prisma_client_1.default.cooler_sub_types.update({
                where: { id: Number(id) },
                data,
            });
            res.json({
    ...
    async deleteCoolerSubType(req, res) {
        try {
            const { id } = req.params;
            const existingCoolerSubType = await prisma_client_1.default.cooler_sub_types.findUnique({
                where: { id: Number(id) },
            ...
            await prisma_client_1.default.cooler_sub_types.delete({ where: { id: Number(id) } });
    ...
    async getCoolerSubTypesDropdown(req, res) {
        try {
...
//# sourceMappingURL=coolerSubTypes.controller.js.map