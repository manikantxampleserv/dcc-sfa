...
const serializeCreditNote = (cn) => ({
    ...
    creditNoteItems: cn.credit_notes_items
        ? cn.credit_notes_items.map((item) => ({
            id: item.id,
...
async function generateCreditNoteNumber(tx) {
    const maxRetries = 10;
    let retryCount = 0;
    while (retryCount < maxRetries) {
        try {
            ...
            if (!exists) {
                console.log(' Generated unique credit note number:', newCreditNoteNumber);
                return newCreditNoteNumber;
            }
            console.log(' Credit note number exists, retrying...', newCreditNoteNumber);
            retryCount++;
        }
        catch (error) {
...
exports.creditNotesController = {
    async upsertCreditNote(req, res) {
        try {
            const data = req.body;
            ...
            let creditNote;
            if (isUpdate) {
                const existingNote = await prisma_client_1.default.credit_notes.findUnique({
                    where: { id: data.id },
                ...
                const existingItemIds = existingNote.credit_notes_items.map(item => item.id);
                const incomingItemIds = creditNoteItems
                    .filter(item => item.id)
                    .map(item => item.id);
                const itemsToDelete = existingItemIds.filter(id => !incomingItemIds.includes(id));
                creditNote = await prisma_client_1.default.$transaction(async (tx) => {
                    ...
                    const processedData = {
                        credit_note_number: data.credit_note_number || existingNote.credit_note_number,
                        ...
                        discount_amount: data.discount_amount ?? existingNote.discount_amount,
                        tax_amount: data.tax_amount ?? existingNote.tax_amount,
                        shipping_amount: data.shipping_amount ?? existingNote.shipping_amount,
    ...
    async createCreditNotes(req, res) {
        try {
            const data = req.body;
            if (!data.parent_id || !data.customer_id) {
                return res
            ...
            const creditNote = await prisma_client_1.default.credit_notes.create({
                data: {
                    ...processedData,
                    createdate: new Date(),
    ...
    async getAllCreditNotes(req, res) {
        try {
            ...
            const { data, pagination } = await (0, paginate_1.paginate)({
                model: prisma_client_1.default.credit_notes,
                filters,
                page: pageNum,
                limit: limitNum,
            ...
            const totalCreditNotes = await prisma_client_1.default.credit_notes.count();
            const activeCreditNotes = await prisma_client_1.default.credit_notes.count({
                where: { is_active: 'Y' },
            });
            const inactiveCreditNotes = await prisma_client_1.default.credit_notes.count({
                where: { is_active: 'N' },
            });
            const now = new Date();
            const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 1);
            const endOfMonth = new Date(now.getFullYear(), now.getMonth() + 1, 1);
            const newCreditNotesThisMonth = await prisma_client_1.default.credit_notes.count({
                where: {
                    createdate: {
                        gte: startOfMonth,
                        lt: endOfMonth,
            ...
            res.success('Credit Notes retrieved successfully', data.map((cn) => serializeCreditNote(cn)), 200, pagination, {
    ...
    async getCreditNoteById(req, res) {
        try {
            const { id } = req.params;
            const creditNote = await prisma_client_1.default.credit_notes.findUnique({
                where: { id: Number(id) },
                include: {
                    credit_notes_customers: true,
                    credit_notes_products: true,
    ...
    async updateCreditNotes(req, res) {
        try {
            const { id } = req.params;
            const existingNote = await prisma_client_1.default.credit_notes.findUnique({
                where: { id: Number(id) },
            ...
            const updatedNote = await prisma_client_1.default.credit_notes.update({
                where: { id: Number(id) },
                data: {
                    ...processedData,
                    updatedate: new Date(),
    ...
    async deleteCreditNotes(req, res) {
        try {
            const { id } = req.params;
            const existingNote = await prisma_client_1.default.credit_notes.findUnique({
                where: { id: Number(id) },
            ...
            await prisma_client_1.default.credit_notes.delete({ where: { id: Number(id) } });
...
//# sourceMappingURL=creditNotes.controller.js.map