...
exports.customerCategoryController = {
    ...
    async bulkCustomerCategory(req, res) {
        try {
            ...
            if (!Array.isArray(req.body)) {
                const singleResult = await prisma_client_1.default.$transaction(async (tx) => {
                    const item = req.body;
                    const duplicateCategory = await tx.customer_category.findFirst({
                        where: {
                            category_name: item.category_name,
                    ...
                    if (duplicateCategory) {
                        throw new Error(`Category name '${item.category_name}' already exists`);
                    }
                    const duplicateLevel = await tx.customer_category.findFirst({
                        where: {
                            level: item.level,
                    ...
                    if (duplicateLevel) {
                        throw new Error(`Level '${item.level}' is already assigned to another category (ID: ${duplicateLevel.id})`);
                    }
                    let parent;
                    if (item.id) {
                        const exists = await tx.customer_category.findUnique({
                    ...
                    const parentId = parent.id;
                    if (item.id) {
                        const existingConditions = await tx.customer_category_condition.findMany({
                        ...
                        const inputConditionIds = (item.conditions || [])
                            .filter(cond => cond.id)
                            .map(cond => cond.id);
                        const conditionsToDelete = existingConditionIds.filter(id => !inputConditionIds.includes(id));
                        if (conditionsToDelete.length > 0) {
                            await tx.customer_category_condition.deleteMany({
    ...
    async deleteCustomerCategory(req, res) {
        try {
            ...
            const [customersCount, conditionsCount, promotionCategoriesCount] = await Promise.all([
                prisma_client_1.default.customers.count({ where: { customer_category_id: id } }),
                prisma_client_1.default.customer_category_condition.count({
                    where: { customer_category_id: id },
                }),
                prisma_client_1.default.promotion_customer_category.count({
                    where: { customer_category_id: id },
                }),
            ]);
            const hasRelatedRecords = customersCount > 0 ||
                conditionsCount > 0 ||
            ...
            if (hasRelatedRecords) {
                ...
                if (promotionCategoriesCount > 0)
                    relatedRecords.push(`${promotionCategoriesCount} promotion category reference(s)`);
                return res.status(400).json({
                    message: 'Cannot delete customer category. This category has related records.',
                    details: {
                        category: exists.category_name,
                        relatedRecords,
                        suggestion: 'Please delete or reassign the related records first, or consider marking this category as inactive instead of deleting.',
                    },
                });
            }
            ...
            if (e.code === 'P2003') {
                ...
                return res.status(400).json({
                    message: 'Cannot delete customer category. This category has related records in other tables.',
                    details: {
                        error: 'Foreign key constraint violated',
                        suggestion: 'Please delete the related records first or mark the category as inactive.',
                    },
                });
            }
            return res.status(500).json({ message: e.message });
    ...
    async assignCategoriesToCustomers(req, res) {
        try {
            ...
            console.log(`Found ${validCategories.length} category levels:`, validCategories);
            ...
            return res.status(200).json({
                ...
                details: results.details.filter(d => d.status === 'updated' || d.status === 'failed'),
            });
        }
        catch (e) {
    ...
    async assignCategoryToSingleCustomer(req, res) {
        try {
    ...
    async getCategoryAssignmentStats(req, res) {
        try {
            ...
            const customersWithoutCategory = await prisma_client_1.default.customers.count({
                where: {
                    is_active: 'Y',
            ...
            const stats = await Promise.all(categories.map(async (cat) => {
                const customerCount = await prisma_client_1.default.customers.count({
                    where: {
                ...
                return {
                    ...
                    level: cat.level,
                    threshold: cat.customer_category_condition_customer_category[0]
                        ?.threshold_value || 0,
                    customerCount,
                };
            }));
