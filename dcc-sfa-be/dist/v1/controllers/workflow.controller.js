...
exports.workflowController = {
    ...
    async getWorkflowSteps(req, res) {
        try {
            ...
            const steps = await returnWorkflow_service_1.ReturnWorkflowService.getWorkflowSteps(Number(requestId));
            res.json({
     ...
     * @param res - Express response object
     */
    async executeWorkflowAction(req, res) {
        try {
            const { requestId } = req.params;
            const { action, remarks, actionBy } = req.body;
            const userId = req.user?.id || 1;
            // Get current return request
            const returnRequest = await prisma_client_1.default.return_requests.findUnique({
            ...
            const updatedRequest = await prisma_client_1.default.return_requests.update({
                where: { id: Number(requestId) },
                data: {
                    status: newStatus,
                    ...
                    approved_date: action === 'approve' ? new Date() : returnRequest.approved_date,
            ...
            const existingStep = await returnWorkflow_service_1.ReturnWorkflowService.getWorkflowStepByStep(Number(requestId), stepName);
            if (existingStep) {
                await returnWorkflow_service_1.ReturnWorkflowService.updateWorkflowStep(existingStep.id, stepStatus, remarks, actionBy || userId);
            }
            else {
                await returnWorkflow_service_1.ReturnWorkflowService.addWorkflowStep(Number(requestId), stepName, stepStatus, remarks, actionBy || userId, userId);
            }
            // Get updated workflow steps
            const workflowSteps = await returnWorkflow_service_1.ReturnWorkflowService.getWorkflowSteps(Number(requestId));
            res.json({
    ...
    async applyWorkflowTemplate(req, res) {
        try {
            ...
            await prisma_client_1.default.return_workflow.deleteMany({
                where: {
            ...
            for (let i = 0; i < template.steps.length; i++) {
                ...
                const newStep = await returnWorkflow_service_1.ReturnWorkflowService.addWorkflowStep(Number(requestId), step, status, i === 0 ? 'Return request submitted by customer' : '', i === 0 ? userId : 1, userId);
     ...
     * @param res - Express response object
     */
    async executeFullWorkflowFlow(req, res) {
        try {
            const { requestId } = req.params;
            const { templateId } = req.body;
            const userId = req.user?.id || 1;
            ...
            const workflowSteps = await returnWorkflow_service_1.ReturnWorkflowService.executeFullWorkflowFlow(Number(requestId), userId, templateId || 'standard_return');
            res.json({
    ...
    async rejectReturnRequest(req, res) {
        try {
            ...
            const rejectionStep = await returnWorkflow_service_1.ReturnWorkflowService.rejectReturnRequest(Number(requestId), userId, rejectionReason.trim());
            res.json({
     ...
     * @param res - Express response object
     */
    async executeNextWorkflowStep(req, res) {
        try {
            const { requestId } = req.params;
            const userId = req.user?.id || 1;
            // Check if return request exists and is not in final state
            ...
            const nextStep = await returnWorkflow_service_1.ReturnWorkflowService.executeNextWorkflowStep(Number(requestId), userId);
            if (!nextStep) {
                return res.json({
