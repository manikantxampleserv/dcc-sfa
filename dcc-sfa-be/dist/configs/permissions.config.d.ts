 ...
 */
export type Modules = 'dashboard' | 'company' | 'user' | 'role' | 'depot' | 'zone' | 'currency' | 'route' | 'route-type' | 'outlet' | 'outlet-group' | 'asset-type' | 'asset-sub-types' | 'asset-master' | 'warehouse' | 'vehicle' | 'brand' | 'product-category' | 'product-sub-category' | 'unit-of-measurement' | 'sub-unit-of-measurement' | 'product-flavour' | 'product-volume' | 'product-shelf-life' | 'product' | 'pricelist' | 'sales-target-group' | 'sales-target' | 'sales-bonus-rule' | 'kpi-target' | 'survey' | 'promotions' | 'order' | 'delivery' | 'return' | 'payment' | 'invoice' | 'credit-note' | 'visit' | 'asset-movement' | 'maintenance' | 'installation' | 'inspection' | 'van-stock' | 'stock-movement' | 'stock-transfer' | 'batch-lots' | 'inventory-management' | 'competitor' | 'customer-complaint' | 'customer-category' | 'customer-type' | 'location' | 'route-effectiveness' | 'erp-sync' | 'report' | 'approval' | 'exception' | 'alert' | 'profile' | 'login-history' | 'token' | 'setting' | 'product-type' | 'product-target-group' | 'product-web-order' | 'inventory-items' | 'asset-master-brands';
/**
 * @type Actions
