...
exports.seedOutletGroups = seedOutletGroups;
exports.clearOutletGroups = clearOutletGroups;
const logger_1 = __importDefault(require("../../configs/logger"));
const prisma_client_1 = __importDefault(require("../../configs/prisma.client"));
const mockOutletGroups = [
    {
        name: 'Premium Retailers',
        code: 'GRP-PREM-RET',
        description: 'High-value retail customers with premium service requirements',
 ...
 */
async function seedOutletGroups() {
    try {
        let groupsCreated = 0;
        let groupsSkipped = 0;
        for (const group of mockOutletGroups) {
            const existingGroup = await prisma_client_1.default.customer_groups.findFirst({
                where: { code: group.code },
            });
            if (!existingGroup) {
                await prisma_client_1.default.customer_groups.create({
                    data: {
                        name: group.name,
                        code: group.code,
        ...
        logger_1.default.info(`Outlet Groups seeding completed: ${groupsCreated} created, ${groupsSkipped} skipped`);
    }
    catch (error) {
        logger_1.default.error('Error seeding outlet groups:', error);
        throw error;
    }
}
/**
 ...
 */
async function clearOutletGroups() {
    try {
        await prisma_client_1.default.customer_groups.deleteMany({});
    }
    catch (error) {
        throw error;
    }
}
//# sourceMappingURL=outlet-groups.seeder.js.map