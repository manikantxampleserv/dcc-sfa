 ...
 */
export type Modules = 'executive-dashboard' | 'grading-dashboard' | 'user' | 'role' | 'depot' | 'zone' | 'currency' | 'route' | 'route-type' | 'outlet' | 'outlet-group' | 'asset-type' | 'asset-sub-types' | 'asset-master' | 'vehicle' | 'brand' | 'product-category' | 'product-sub-category' | 'unit-of-measurement' | 'product-flavour' | 'product-volume' | 'product-shelf-life' | 'product' | 'pricelist' | 'sales-target-group' | 'sales-target' | 'sales-bonus-rule' | 'kpi-target' | 'survey' | 'promotions' | 'order' | 'return' | 'payment' | 'invoice' | 'visit' | 'asset-movement' | 'maintenance' | 'installation' | 'inspection' | 'van-stock' | 'stock-movement' | 'competitor' | 'customer-complaint' | 'customer-category' | 'customer-type' | 'location' | 'route-effectiveness' | 'report' | 'approval' | 'exception' | 'login-history' | 'token' | 'setting' | 'product-type' | 'product-target-group' | 'product-web-order' | 'inventory-items' | 'asset-master-brands' | 'region' | 'district' | 'city' | 'tax-master' | 'templates';
/**
 * @type Actions
