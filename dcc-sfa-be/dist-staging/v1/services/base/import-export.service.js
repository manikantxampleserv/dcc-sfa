...
const prisma_client_1 = __importDefault(
  require('../../../configs/prisma.client')
);
class ImportExportService {
  ...
  async parseExcelFile(buffer) {
    const errorHandler =
      new import_export_error_service_1.ImportExportErrorHandler(this.columns);
    const validData = [];
    try {
  ...
  async importData(data, userId, options = {}) {
    ...
    const results = await prisma_client_1.default.$transaction(
      async tx => {
