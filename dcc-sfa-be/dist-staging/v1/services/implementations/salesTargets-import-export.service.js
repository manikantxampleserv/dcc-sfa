...
exports.SalesTargetsImportExportService = void 0;
const import_export_service_1 = require("../base/import-export.service");
const ExcelJS = __importStar(require("exceljs"));
const prisma_client_1 = __importDefault(require("../../../configs/prisma.client"));
class SalesTargetsImportExportService extends import_export_service_1.ImportExportService {
    modelName = 'sales_targets';
    ...
    searchFields = ['target_quantity', 'target_amount'];
    columns = [
        {
            ...
            validation: value => {
                if (!value)
                    return 'Sales target group ID is required';
        ...
        },
        {
            ...
            transform: value => parseInt(value),
            description: 'Target quantity for the sales target (required, positive integer)',
        },
        {
            key: 'target_amount',
            ...
            validation: value => {
                if (value) {
                    ...
                    if (num < 0)
                        return 'Target amount must be non-negative';
                }
                return true;
            },
            transform: value => (value ? parseFloat(value) : null),
            description: 'Target amount for the sales target (optional, non-negative)',
        },
        {
            key: 'start_date',
            header: 'Start Date',
            ...
            description: 'Start date of the sales target period (required, format: YYYY-MM-DD)',
        },
        {
            key: 'end_date',
            header: 'End Date',
            ...
            transform: value => new Date(value),
            description: 'End date of the sales target period (required, format: YYYY-MM-DD)',
        },
        {
            key: 'is_active',
            header: 'Is Active',
...
- No overlapping targets allowed for the same group and category in the same period.
    ...
    async transformDataForExport(data) {
        return data.map(target => ({
            ...
            product_category_name: target.sales_targets_product_categories?.category_name || '',
    ...
    async checkDuplicate(data, tx) {
        const model = tx ? tx.sales_targets : prisma_client_1.default.sales_targets;
    ...
    async validateForeignKeys(data) {
        ...
        const existingGroups = await prisma_client_1.default.sales_target_groups.findMany({
        ...
        const existingCategories = await prisma_client_1.default.product_categories.findMany({
    ...
    async exportToExcel(options = {}) {
        ...
        exportData.forEach((row, index) => {
            const excelRow = worksheet.addRow({
                ...
                category_name: data[index]?.sales_targets_product_categories?.category_name || '',
            });
            if (index % 2 === 0) {
                excelRow.fill = {
                    type: 'pattern',
...
exports.SalesTargetsImportExportService = SalesTargetsImportExportService;
//# sourceMappingURL=salesTargets-import-export.service.js.map