...
const serializeRole = (role, includeCreatedAt = false, includeUpdatedAt = false) => ({
    id: role.id,
    name: role.name,
...
exports.rolesController = {
    async createRole(req, res) {
        try {
            ...
            const { name, description, type, is_assigned_route, 
            // user_id,
            is_active, unload_time, permissions = [], } = req.body;
    ...
    async getRoleById(req, res) {
        try {
            ...
            const role = await prisma_client_1.default.roles.findFirst({
                where: {
                    id: id,
                    is_active: 'Y',
                },
    ...
    async updateRole(req, res) {
        try {
            ...
            if (roleData.name && roleData.name !== existingRole.name) {
                const nameExists = await prisma_client_1.default.roles.findFirst({
                    where: {
                        name: roleData.name,
                        id: { not: id },
                    },
    ...
    async deleteRole(req, res) {
        try {
            ...
            const existingRole = await prisma_client_1.default.roles.findUnique({
                where: {
                    id: parseInt(id),
                },
            });
            ...
            const usersWithRole = await prisma_client_1.default.users.count({
                where: {
                    role_id: parseInt(id),
                    is_active: 'Y',
                },
    ...
    async getAllRoles(req, res) {
        try {
            ...
            const { data, pagination } = await (0, paginate_1.paginate)({
                model: prisma_client_1.default.roles,
                filters,
                page: page_num,
                limit: limit_num,
            ...
            const totalRoles = await prisma_client_1.default.roles.count();
            const activeRoles = await prisma_client_1.default.roles.count({
                where: { is_active: 'Y' },
            });
            const inactiveRoles = await prisma_client_1.default.roles.count({
                where: { is_active: 'N' },
            });
            ...
            const newRolesThisMonth = await prisma_client_1.default.roles.count({
                where: {
                    createdate: {
                        gte: startOfMonth,
                        lt: endOfMonth,
            ...
            res.success('Roles retrieved successfully', data.map((role) => serializeRole(role, true, true)), 200, pagination, stats);
        }
        catch (error) {
    ...
    async getRolePermissions(req, res) {
        try {
            ...
            const id = Number(req.params.id);
            const permissions = await prisma_client_1.default.role_permissions.findMany({
                where: {
                    role_id: id,
                    is_active: 'Y',
                },
            ...
            res.success('Role permissions fetched successfully', permissions.map(p => p.permission_id), 200);
        }
        catch (error) {
