...
exports.salesControlTowerController = {
    ...
    async exportSalesData(req, res) {
        try {
            const { startDate, endDate, depot_id, route_id, salesman_id, brand_id, pack, channel, } = req.query;
            ...
            for (const inv of invoices) {
                ...
                const route = inv.invoices_customers?.customer_routes?.name ||
                    inv.invoices_salesperson?.route_salespersons?.[0]?.route
                        ?.name ||
                    '';
                const salesman = inv.invoices_salesperson?.name || '';
                ...
                for (const item of inv.invoice_items) {
                    ...
                    flatRows.push({
                        ...
                        UnitCase: Number(item.quantity) * Number(item.conversion_factor || 1),
