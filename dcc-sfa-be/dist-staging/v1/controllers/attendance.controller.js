...
const serializeAttendance = (attendance) => ({
    ...
    attendance_history: attendance.attendance_history
        ? attendance.attendance_history.map((h) => ({
            ...h,
...
exports.attendanceController = {
    async punch(req, res) {
        try {
            ...
            const { action_type, latitude, longitude, address, workType, deviceInfo, remarks, } = req.body;
            if (!action_type) {
            ...
            if (action_type === 'punch_in') {
                let existingAttendance = await prisma_client_1.default.attendance.findFirst({
                    where: {
                        user_id: userId,
                        is_active: 'Y',
                    },
                ...
                if (!existingAttendance || existingAttendance.status === 'punch_out') {
                    ...
                    attendance = await prisma_client_1.default.attendance.create({
                        data: {
                            user_id: userId,
                ...
                const existingAttendance = await prisma_client_1.default.attendance.findFirst({
                    where: {
                        user_id: userId,
                        is_active: 'Y',
                    },
                ...
                attendance = await prisma_client_1.default.attendance.update({
    ...
    async getPunchStatus(req, res) {
        try {
            const userId = req.user?.id || 0;
            const attendance = await prisma_client_1.default.attendance.findFirst({
                where: {
                    user_id: userId,
                    is_active: 'Y',
                },
            ...
            res.status(200).json({
                message: 'Punch status retrieved successfully',
                data: {
                    status: (attendance.status || 'punch_in'),
