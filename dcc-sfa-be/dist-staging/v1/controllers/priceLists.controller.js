...
const serializePriceList = (pl) => ({
    ...
    pricelist_item: (pl.pricelist_item || []).map((item) => {
        const rawSpecialPrices = item.pricelist_item_special_prices || item.special_prices || [];
        const specialPrices = rawSpecialPrices.map((sp) => ({
            id: sp.id,
...
exports.priceListsController = {
    async upsertPriceList(req, res) {
        ...
        try {
            ...
            if (data.is_default === 'Y') {
                ...
                const existingDefault = await prisma_client_1.default.pricelists.findFirst({
                ...
                for (const item of items) {
                    const itemData = {
                        ...
                        discount_percent: item.discount_percent !== '' && item.discount_percent !== null
                            ...
                            : null,
                        tax_percent: item.tax_percent !== '' && item.tax_percent !== null
                    ...
                    if (item.id && existingItems.find(e => e.id === item.id)) {
                        const updatedItem = await prisma_client_1.default.pricelist_items.update({
                            where: { id: item.id },
                            data: {
                                ...itemData,
                                updatedate: new Date(),
                        ...
                        if (Array.isArray(item.special_prices)) {
                            const existingSpecialPrices = await prisma_client_1.default.pricelist_item_special_prices.findMany({
                            ...
                            for (const sp of item.special_prices) {
                                ...
                                if (sp.id && existingSpecialPrices.find(e => e.id === sp.id)) {
                                    await prisma_client_1.default.pricelist_item_special_prices.update({
                                        where: { id: sp.id },
                                        data: {
                                            ...spData,
                                            updatedate: new Date(),
                                    ...
                                    await prisma_client_1.default.pricelist_item_special_prices.create({
                                        data: {
                                            ...spData,
                                            pricelist_item_id: updatedItem.id,
                                            createdate: new Date(),
                        ...
                        const newItem = await prisma_client_1.default.pricelist_items.create({
                            data: {
                                ...itemData,
                                pricelist_id: priceList.id,
                                createdate: new Date(),
    ...
    async getAllPriceLists(req, res) {
        try {
            const { page, limit, search, status, depot_id, route_id, customer_id, customer_category_id, include_items, is_default, } = req.query;
            ...
            const data = await prisma_client_1.default.pricelists.findMany({
                where: filters,
            ...
            const totalCount = await prisma_client_1.default.pricelists.count({ where: filters });
            const pagination = {
            ...
            const defaultPriceLists = await prisma_client_1.default.pricelists.count({
                where: { is_default: 'Y' },
            });
            const now = new Date();
    ...
    async getPriceListByCustomer(req, res) {
        try {
