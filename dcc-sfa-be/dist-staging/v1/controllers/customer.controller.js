...
const serializeCustomer = async (customer, defaultOutletIdSet) => {
    ...
    return {
        ...
        default_for_depots: customer.default_for_depots
            ? customer.default_for_depots.map((d) => ({
                id: d.id,
                name: d.name,
                code: d.code,
            }))
...
exports.customerController = {
    ...
    async bulkUpsertCustomers(req, res) {
        try {
            ...
            const platform_type = req.headers['platform_type'] || req.body.platform_type || 'web';
            let customersData;
                ...
                for (let customerIndex = 0; customerIndex < customersData.length; customerIndex++) {
                    ...
                    try {
                        ...
                        if (!existingCustomer) {
                            ...
                            try {
                                const customerCreationRequest = await (0, requests_controller_1.createRequest)({
                                ...
                                const requestWithApprovals = await prisma_client_1.default.sfa_d_requests.findUnique({
                                ...
                                if (!requestWithApprovals?.sfa_d_requests_approvals_request ||
                                    ...
                                    console.log(`Customer created directly: ${createdCustomer.code}`);
                                }
                                else {
    ...
    async createLocationChangeRequest(req, res) {
        try {
            if (!req.body) {
                return res.status(400).json({
                    success: false,
            ...
            const { customer_id, customer_code, latitude, longitude, reason, request_type = 'LOCATION_RESET', } = req.body;
            ...
            if (customer_id) {
                existingCustomer = await prisma_client_1.default.customers.findUnique({
            ...
            const locationResetRequest = await (0, requests_controller_1.createRequest)({
                requester_id: req.user?.id || 1,
    ...
    async createCustomers(req, res) {
        try {
            ...
            const { credit_limit, outstanding_amount, latitude, longitude, is_default_for_depot, ...otherData } = data;
            const processedData = {
                ...otherData,
            ...
            if (is_default_for_depot && customer.depot_id) {
                await prisma_client_1.default.depots.update({
                    where: { id: customer.depot_id },
                    data: { default_outlet_id: customer.id },
                });
            }
            // Re-fetch to get correct relations
            const finalCustomer = await prisma_client_1.default.customers.findUnique({
    ...
    async updateCustomers(req, res) {
        try {
            ...
            const { credit_limit, outstanding_amount, latitude, longitude, is_default_for_depot, ...otherData } = req.body;
            const data = {
                ...otherData,
                credit_limit: credit_limit === '' ? null : credit_limit,
                outstanding_amount: outstanding_amount === '' ? null : outstanding_amount,
