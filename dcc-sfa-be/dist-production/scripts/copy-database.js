...
const SOURCE_DB_URL = 'sqlserver://10.160.5.101:1433;initial catalog=DCC_SFA2_LIVE;user=sa;password=Click@321$;TrustServerCertificate=true;';
...
async function copyTable(sourcePrisma, targetPrisma, tableName) {
    try {
        ...
        try {
            await targetPrisma.$executeRawUnsafe(`SET IDENTITY_INSERT ${tableName} ON`);
        ...
        if (identityInsertEnabled) {
            try {
                await targetPrisma.$executeRawUnsafe(`SET IDENTITY_INSERT ${tableName} OFF`);
            }
            catch (e) {
                // Ignore error
            }
...
async function clearTable(targetPrisma, tableName) {
    try {
        ...
        try {
            await targetPrisma.$executeRawUnsafe(`DBCC CHECKIDENT ('${tableName}', RESEED, 1)`);
        }
        catch (e) {
...
copyDatabase();
//# sourceMappingURL=copy-database.js.map