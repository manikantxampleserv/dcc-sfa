...
export class DepotsImportExportService extends ImportExportService {
    constructor() {
        super();
        this.modelName = 'depots';
        this.displayName = 'Depots';
        this.uniqueFields = ['code'];
        this.searchFields = ['name', 'code', 'address', 'city', 'email'];
        this.masterTableConfigs = [
            {
                ...
                sheetName: 'Companies',
                description: 'Reference for Company IDs',
            },
        ];
        this.columns = [
            {
            ...
            },
            {
                ...
                description: 'State/Province where depot is located (optional, max 100 chars)',
            },
            {
                key: 'zipcode',
                header: 'Zip Code',
    ...
    async getDepotsWithRelations(filters) {
    ...
    // Override generateTemplate to include master table data
    async generateTemplate() {
        const ExcelJS = require('exceljs');
        const workbook = new ExcelJS.Workbook();
        // 1. Main depots template sheet
        ...
        if (companiesData.length > 0) {
            ...
            // Style header
            const companyHeader = companySheet.getRow(1);
            companyHeader.font = { bold: true, color: { argb: 'FFFFFFFF' } };
            companyHeader.fill = {
                type: 'pattern',
                pattern: 'solid',
                fgColor: { argb: 'FF4472C4' },
            };
            companyHeader.alignment = { vertical: 'middle', horizontal: 'center' };
            companyHeader.height = 25;
            // Add companies data
            companiesData.forEach((company, index) => {
                const row = companySheet.addRow(company);
                row.eachCell(cell => {
                    cell.border = {
                        top: { style: 'thin' },
                        left: { style: 'thin' },
        ...
        instructionSheet.addRow([
            'Use following sheets for reference data:',
            '',
            '',
            '',
        ]);
        instructionSheet.addRow([
            '- Companies: Reference for Company IDs',
            '',
            '',
            '',
        ]);
        ...
        const instructions = [
            '1. Do not modify column headers in template sheet',
