...
exports.sapService = {
    async createOrUpdateVanInventorySAP(payload, userId = 1) {
        ...
        // Create approval request if workflow exists and it's a new or pending update
        if (workflowExists && finalResult.finalInventory.approval_status === 'P') {
            const formattedVanInventoryItems = items.map((item) => {
                const product = finalResult.finalInventory.van_inventory_items_inventory?.find((i) => i.sap_item_code === item.product_sap_code)?.van_inventory_items_products;
                return {
                    product_id: product?.id,
                    product_name: product?.name,
                    tracking_type: product?.tracking_type,
                    ...
                    sap_lineid: item.sap_lineid,
                    source_system: item.source_system,
                    product_batches: item.batches?.map((b) => ({
                        batch_number: b.batch_number,
                        lot_number: b.lot_number || '',
                        manufacturing_date: b.manufacturing_date,
                        expiry_date: b.expiry_date,
                    ...
                    product_serials: item.serials?.map((s) => ({
                        serial_number: s.serial_number || s,
                        quantity: s.quantity || 1,
                    })) || [],
                };
