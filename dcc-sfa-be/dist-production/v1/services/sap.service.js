...
exports.sapService = {
            ...
            if (!inventoryId) {
                const existingAnyInventory = await tx.van_inventory.findFirst({
                    where: {
                        user_id: Number(inventoryData.user_id),
                        document_date: {
                            gte: new Date(normalizedDocDate.getFullYear(), normalizedDocDate.getMonth(), normalizedDocDate.getDate()),
                            lt: new Date(normalizedDocDate.getFullYear(), normalizedDocDate.getMonth(), normalizedDocDate.getDate() + 1),
                        },
                    },
                    orderBy: {
                        createdate: 'desc',
                ...
                if (existingAnyInventory) {
                    ...
                    if (effectiveApprovalStatus === 'P' || loadingType === 'U') {
                        ...
                        throw new Error(`Cannot create new van inventory. There is already an existing inventory (ID: ${existingAnyInventory.id}) with approval_status '${effectiveApprovalStatus}' for this user and document date. Only pending (P) inventories or unloading operations are allowed.`);
                    }
                }
                else {
                    ...
                    if (loadingType === 'L') {
                        if (trackingType === 'BATCH') {
                                        ...
                                        batchLot = await tx.batch_lots.create({
                                            data: {
                                                ...
                                                expiry_date: batchInput.expiry_date
                                                    ? new Date(batchInput.expiry_date)
                                                    : new Date(new Date().setFullYear(new Date().getFullYear() + 2)),
                                                quantity: 0, // Don't add to stock since we're skipping loading
                                                remaining_quantity: 0, // Don't add to remaining quantity
                                                supplier_name: batchInput.supplier_name || null,
                                                purchase_price: batchInput.purchase_price || null,
                                                quality_grade: batchInput.quality_grade || 'A',
                                                storage_location: batchInput.storage_location || null,
                            ...
                            for (const serialInput of serialData) {
                                ...
                                // Only modify serial numbers if we're performing loading/unloading AND item isn't cancelled
                                if (shouldPerformLoadingUnloading && !itemIsCancelled) {
                                    if (existingSerial) {
                                        if (existingSerial.status === 'in_van') {
                                            throw new Error(`Serial ${serialNumber} is already loaded to van and cannot be loaded again until it becomes available`);
                                    ...
                                    // If we aren't modifying stock, still create the serial if it doesn't exist
                                    if (!existingSerial) {
                                        existingSerial = await tx.serial_numbers.create({
                                            data: {
                                                product_id: product.id,
                        ...
                        if (trackingType === 'BATCH') {
                            ...
                            const declaredQtyUnload = parseInt(item.quantity, 10);
                            if (Number.isNaN(declaredQtyUnload) ||
                                declaredQtyUnload !== totalBatchQtyUnload) {
                                throw new Error(`Quantity mismatch for batch-tracked product "${product.name}": declared quantity ${item.quantity} does not match sum of batches ${totalBatchQtyUnload}`);
                            }
                            for (const batchInput of batchData) {
                                const batchQty = parseInt(batchInput.quantity, 10) || 0;
                                const batchLot = await tx.batch_lots.findFirst({
        ...
        if (workflowExists && finalResult.finalInventory.approval_status === 'P') {
            await (0, requests_controller_1.createRequest)({
                ...
                request_data: JSON.stringify({
                    // Store full original data including items for later processing
                    ...inventoryData,
                    items,
                }),
                createdby: userId,
    ...
    async processApprovedVanInventoryStock(inventoryId, userId = 1) {
        ...
        const inventory = await prisma_client_1.default.van_inventory.findUnique({
            where: { id: Number(inventoryId) },
            // We don't need to include items here since we're getting the original payload
        });
        if (!inventory) {
            throw new Error(`Van inventory not found with ID: ${inventoryId}`);
        }
        // Get the original approval request to retrieve the full payload including items
