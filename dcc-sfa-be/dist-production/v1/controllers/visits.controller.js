    ...
    async bulkUpsertVisits(req, res) {
        try {
            ...
            for (let index = 0; index < dataArray.length; index++) {
                ...
                try {
                    const { visit, invoices, payments, cooler_inspections, cooler_installations, survey, } = data;
                    if (!visit) {
                        results.failed.push({
                            visitIndex: index,
                            data,
                                        ...
                                        for (const item of invoiceItems) {
                                            ...
                                            if (trackingType === 'BATCH') {
                                                ...
                                                if (hasBatchNumber) {
                                                    ...
                                                    if (!batchLot) {
                                                        throw new Error(`Batch "${batchNumber}" not found for product "${product.name}" assigned to salesperson`);
                                                    }
                                                    batchDeductions = [
                                                        {
