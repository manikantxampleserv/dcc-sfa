...
async function getReportData(filters) {
    const { start_date, end_date, customer_id, status, depotIds, isScopeRestricted, } = filters;
    const dateFilter = {};
    if (start_date) {
...
exports.reportsController = {
     ...
     */
    async getOrdersInvoicesReturnsReport(req, res) {
        try {
            ...
            const totalInvoiceValue = invoices.reduce((sum, invoice) => sum + Number(invoice.total_amount || 0), 0);
            ...
            const pendingInvoices = invoices.filter(inv => inv.status?.toUpperCase() === 'PENDING' ||
                inv.status?.toUpperCase() === 'UNPAID' ||
                inv.status?.toLowerCase() === 'pending' ||
                inv.status?.toLowerCase() === 'unpaid');
            const completedOrders = orders.filter(order => order.status?.toUpperCase() === 'DELIVERED' ||
                order.status?.toUpperCase() === 'COMPLETED' ||
                order.status?.toLowerCase() === 'delivered' ||
                order.status?.toLowerCase() === 'completed');
            const pendingOrders = orders.filter(order => order.status?.toUpperCase() === 'PENDING' ||
            ...
            const conversionRate = orders.length > 0 ? (invoices.length / orders.length) * 100 : 0;
     ...
     */
    async exportOrdersInvoicesReturnsReport(req, res) {
        try {
            const { start_date, end_date, customer_id, status } = req.query;
            const { orders, invoices } = await getReportData({
                start_date,
            ...
            res.setHeader('Content-Disposition', `attachment; filename=Orders_Invoices_Report_${Date.now()}.xlsx`);
     ...
     */
    async getSalesVsTargetReport(req, res) {
        try {
            const { start_date, end_date, salesperson_id, product_category_id, sales_target_group_id, } = req.query;
            const reqUser = req.user;
            let isScopeRestricted = false;
            let depotIds = [];
            if (reqUser && !(0, permissions_config_1.isAdminRole)(reqUser.role)) {
            ...
            const whereTargets = {
                ...
                ...(sales_target_group_id && {
                    sales_target_group_id: parseInt(sales_target_group_id),
                }),
     ...
     */
    async exportSalesVsTargetReport(req, res) {
        try {
            const { start_date, end_date, salesperson_id, product_category_id, sales_target_group_id, } = req.query;
            const dateFilter = {};
            if (start_date) {
            ...
            const ExcelJS = await Promise.resolve().then(() => __importStar(require('exceljs')));
            const workbook = new ExcelJS.Workbook();
            const performanceSheet = workbook.addWorksheet('Performance by Salesperson', {
                pageSetup: { paperSize: 9, orientation: 'landscape' },
            });
            ...
            categoryPerformanceArray.forEach((row) => {
            ...
            const buffer = await workbook.xlsx.writeBuffer();
            res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
            res.setHeader('Content-Disposition', `attachment; filename=Sales_Vs_Target_Report_${Date.now()}.xlsx`);
            res.setHeader('Content-Length', buffer.byteLength.toString());
            res.send(Buffer.from(buffer));
        }
        catch (error) {
     ...
     */
    async getAssetMovementStatusReport(req, res) {
        try {
            const { start_date, end_date, asset_type_id, asset_status, customer_id } = req.query;
            const reqUser = req.user;
            let isScopeRestricted = false;
            let depotIds = [];
            if (reqUser && !(0, permissions_config_1.isAdminRole)(reqUser.role)) {
            ...
            const whereAssets = {
                ...
                ...(asset_status && { current_status: asset_status }),
            };
            if (isScopeRestricted) {
                if (depotIds.length > 0) {
            ...
            const whereCustomerAssets = {
                is_active: 'Y',
                ...(customer_id && { customer_id: parseInt(customer_id) }),
                ...(asset_type_id && {
                    asset_type_id: parseInt(asset_type_id),
                }),
                ...(asset_status && { status: asset_status }),
            };
            const customerAssets = await prisma_client_1.default.customer_assets.findMany({
            ...
            const warrantyClaims = await prisma_client_1.default.asset_warranty_claims.findMany({
                where: whereClaims,
                include: {
            ...
            const serializedAssets = assets.map(asset => ({
                ...
                brand: asset.asset_master_brands?.name ||
                ...
                current_location: asset.asset_master_depot?.name ||
            ...
            const serializedMovements = movements.map(movement => ({
                ...
                performed_by_email: movement.asset_movements_performed_by?.email || 'N/A',
            ...
            const serializedWarrantyClaims = warrantyClaims.map(claim => ({
                id: claim.id,
                asset_serial: claim.asset_master_warranty_claims?.serial_number || 'N/A',
                asset_type: claim.asset_master_warranty_claims?.asset_master_asset_types?.name ||
                    'N/A',
            ...
            const summary = {
                ...
                assets_by_status: assets.reduce((acc, asset) => {
                    const status = asset.current_status || 'Unknown';
                    acc[status] = (acc[status] || 0) + 1;
                    return acc;
                }, {}),
                customer_assets_by_status: customerAssets.reduce((acc, ca) => {
                    const status = ca.status || 'Unknown';
                    acc[status] = (acc[status] || 0) + 1;
                    return acc;
                }, {}),
                claims_by_status: warrantyClaims.reduce((acc, claim) => {
     ...
     */
    async exportAssetMovementStatusReport(req, res) {
        try {
            const { start_date, end_date, asset_type_id, asset_status, customer_id } = req.query;
            // Build date filter
            const dateFilter = {};
            ...
            movements.forEach((movement) => {
                const from_location = movement.from_direction === 'depot'
                    ? movement.asset_movement_from_depot?.name || 'N/A'
                    : movement.asset_movement_from_customer?.name || 'N/A';
                const to_location = movement.to_direction === 'depot'
                    ? movement.asset_movement_to_depot?.name || 'N/A'
                    : movement.asset_movement_to_customer?.name || 'N/A';
                const approvalStatusLower = String(movement.approval_status || 'P').toLowerCase();
                const approvalLabel = approvalStatusLower === 'p'
                    ? 'Pending'
            ...
            res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
            res.setHeader('Content-Disposition', `attachment; filename=Asset_Movement_Status_Report_${Date.now()}.xlsx`);
            res.setHeader('Content-Length', buffer.byteLength.toString());
            res.send(Buffer.from(buffer));
        }
        catch (error) {
     ...
     */
    async getVisitFrequencyCompletionReport(req, res) {
        try {
            const { start_date, end_date, salesperson_id, customer_id, status } = req.query;
            const reqUser = req.user;
            let isScopeRestricted = false;
            let depotIds = [];
            if (reqUser && !(0, permissions_config_1.isAdminRole)(reqUser.role)) {
            ...
            const whereVisits = {
                ...
                ...(salesperson_id && {
                    sales_person_id: parseInt(salesperson_id),
                }),
                ...(customer_id && { customer_id: parseInt(customer_id) }),
                ...(status && { status: status }),
            };
            ...
            const whereGpsLogs = {
                ...
                ...(salesperson_id && {
                    user_id: parseInt(salesperson_id),
                }),
            };
            const gpsLogs = await prisma_client_1.default.gps_logs.findMany({
                where: whereGpsLogs,
                include: {
            ...
            const serializedVisits = visits.map(visit => {
                ...
                const completedTasks = visit.visit_tasks_visits?.filter((task) => task.status === 'completed').length || 0;
                const duration = visit.end_time && visit.start_time
                    ? Math.round((new Date(visit.end_time).getTime() -
            ...
            visits.forEach(visit => {
                if (visit.visit_tasks_visits && visit.visit_tasks_visits.length > 0) {
                    visit.visit_tasks_visits.forEach((task) => {
                        allTasks.push({
                            id: task.id,
                            customer_name: visit.visit_customers?.name || 'N/A',
                            visit_date: visit.visit_date?.toISOString() || '',
            ...
            const inProgressVisits = visits.filter(v => v.status?.toLowerCase() === 'in_progress').length;
            const plannedVisits = visits.filter(v => v.status?.toLowerCase() === 'planned').length;
            const totalTasks = allTasks.length;
            const completedTasks = allTasks.filter(t => t.status === 'completed').length;
            const pendingTasks = allTasks.filter(t => t.status === 'pending').length;
            const totalDuration = serializedVisits.reduce((sum, v) => sum + v.duration_minutes, 0);
            const avgDuration = serializedVisits.length > 0
                ? Math.round(totalDuration / serializedVisits.length)
                : 0;
            const totalOrdersCreated = serializedVisits.reduce((sum, v) => sum + v.orders_created, 0);
            const totalAmountCollected = serializedVisits.reduce((sum, v) => sum + v.amount_collected, 0);
     ...
     */
    async exportVisitFrequencyCompletionReport(req, res) {
        try {
            const { start_date, end_date, salesperson_id, customer_id, status } = req.query;
            // Build date filter
            const dateFilter = {};
            ...
            visits.forEach((visit) => {
                ...
                const completedTaskCount = visit.visit_tasks_visits?.filter((task) => task.status === 'completed').length || 0;
                visitsSheet.addRow({
                    customer_name: visit.visit_customers?.name || 'N/A',
                    customer_code: visit.visit_customers?.code || 'N/A',
            ...
            const completedVisitsExport = visits.filter(v => v.status?.toLowerCase() === 'completed').length;
            const totalTasksExport = allTasks.length;
            const completedTasksExport = allTasks.filter(t => t.status === 'completed').length;
            const summaryData = [
            ...
            // Set response headers
            res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
            res.setHeader('Content-Disposition', `attachment; filename=Visit_Frequency_Completion_Report_${Date.now()}.xlsx`);
            res.setHeader('Content-Length', buffer.byteLength.toString());
            res.send(Buffer.from(buffer));
        }
        catch (error) {
     ...
     */
    async getPromoEffectivenessReport(req, res) {
        try {
            const { start_date, end_date, promotion_id, depot_id, zone_id } = req.query;
            const reqUser = req.user;
            let isScopeRestricted = false;
            let depotIds = [];
            if (reqUser && !(0, permissions_config_1.isAdminRole)(reqUser.role)) {
            ...
            const serializedPromotions = promotions.map((promo) => {
                ...
                const applicableCustomerTypes = customerTypes
                    .map((ct) => ct.promotion_customer_types_customer?.type_name || 'N/A')
                    .join(', ');
                return {
                    id: promo.id,
                    name: promo.name,
                    ...
                    depot_name: promo.promotion_depot_promotions?.[0]?.depots?.name || 'N/A',
                    zone_name: promo.promotion_zones_promotions?.[0]?.promotion_zones_zones
                        ?.name || 'N/A',
                    total_products: totalProducts,
                    total_parameters: totalParameters,
                    customer_types: applicableCustomerTypes || 'All',
                    is_active: promo.is_active,
                    status: new Date() < promo.start_date
                        ? 'Upcoming'
            ...
            promotions.forEach((promo) => {
                if (promo.products_promotion_products &&
                    ...
                    promo.products_promotion_products.forEach((pp) => {
                        serializedProducts.push({
                            ...
                            product_category_id: pp.promotion_products_products?.category_id || null,
                        });
                    });
                }
            });
            ...
            promotions.forEach((promo) => {
                if (promo.promotion_parameters_promotions &&
                    ...
                    promo.promotion_parameters_promotions.forEach((param) => {
            ...
            const activePromotions = promotions.filter(p => new Date(p.end_date) >= new Date() &&
                new Date(p.start_date) <= new Date()).length;
            ...
            const uniqueCustomerTypes = new Set(promotions.flatMap((p) => p.promotion_customer_types_promotions?.map((ct) => ct.promotion_customer_types_customer?.type_name) || [])).size;
            const summary = {
                ...
                unique_depots: new Set(promotions.map(p => p.depot_id).filter(Boolean))
                    .size,
                unique_zones: new Set(promotions.map((p) => p.zone_id).filter(Boolean)).size,
            };
            const response = {
                summary,
                data: {
     ...
     */
    async exportPromoEffectivenessReport(req, res) {
        try {
            const { start_date, end_date, promotion_id, depot_id, zone_id } = req.query;
            // Build date filter
            const dateFilter = {};
            ...
            promotions.forEach((promo) => {
                ...
                const status = new Date() < promo.start_date
                    ? 'Upcoming'
                    : new Date() > promo.end_date
                        ? 'Expired'
                        : 'Active';
                promotionsSheet.addRow({
                    ...
                    zone: promo.promotion_zones_promotions?.[0]?.promotion_zones_zones
                        ?.name || 'N/A',
            ...
            const allProductsForSummary = [];
            const allParametersForSummary = [];
            promotions.forEach((promo) => {
                if (promo.products_promotion_products) {
                    allProductsForSummary.push(...promo.products_promotion_products);
                }
                if (promo.promotion_parameters_promotions) {
                    allParametersForSummary.push(...promo.promotion_parameters_promotions);
                }
            });
            const summaryData = [
            ...
            // Style header rows
            [promotionsSheet, productsSheet, parametersSheet, summarySheet].forEach(sheet => {
                const headerRow = sheet.getRow(1);
                headerRow.font = { bold: true, color: { argb: 'FFFFFFFF' } };
                headerRow.fill = {
                    type: 'pattern',
            ...
            // Set response headers
            res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
            res.setHeader('Content-Disposition', `attachment; filename=Promo_Effectiveness_Report_${Date.now()}.xlsx`);
            res.setHeader('Content-Length', buffer.byteLength.toString());
            res.send(Buffer.from(buffer));
        }
        catch (error) {
     ...
     */
    async getRegionTerritorySalesReport(req, res) {
        try {
            ...
            const serializedZones = zones.map(zone => {
                const zoneOrders = orders.filter(o => o.orders_customers?.zones_id === zone.id);
                const zoneInvoices = invoices.filter(i => i.invoices_customers?.zones_id === zone.id);
                const zonePayments = payments.filter(p => p.payments_customers?.zones_id === zone.id);
                const totalOrders = zoneOrders.length;
                const totalOrderValue = zoneOrders.reduce((sum, o) => sum + Number(o.total_amount || 0), 0);
                ...
                const routesData = (zone.route_zones || []).map((routeZone) => {
                    ...
                    const routeInvoices = invoices.filter((i) => {
                    ...
                    return {
                        ...
                        invoice_value: routeInvoices.reduce((sum, i) => sum + Number(i.total_amount || 0), 0),
                    };
                });
                return {
                    id: zone.id,
            ...
            const totalCollection = payments.reduce((sum, p) => sum + Number(p.total_amount || 0), 0);
            const summary = {
     ...
     */
    async exportRegionTerritorySalesReport(req, res) {
        try {
            const { start_date, end_date, zone_id, depot_id, route_id } = req.query;
            // Build date filter
            const dateFilter = {};
            ...
            zones.forEach((zone) => {
                ...
                zonesSheet.addRow({
                    ...
                    order_value: zoneOrders.reduce((sum, o) => sum + Number(o.total_amount || 0), 0),
                    invoices: zoneInvoices.length,
                    invoice_value: zoneInvoices.reduce((sum, i) => sum + Number(i.total_amount || 0), 0),
                    collection: zonePayments.reduce((sum, p) => sum + Number(p.total_amount || 0), 0),
                });
            });
            // Create Summary sheet
            const summarySheet = workbook.addWorksheet('Summary');
            ...
            // Set response headers
            res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
            res.setHeader('Content-Disposition', `attachment; filename=Region_Territory_Sales_Report_${Date.now()}.xlsx`);
            res.setHeader('Content-Length', buffer.byteLength.toString());
            res.send(Buffer.from(buffer));
        }
        catch (error) {
     ...
     */
    async getRepProductivityReport(req, res) {
        try {
            const { start_date, end_date, salesperson_id, depot_id, zone_id } = req.query;
            const reqUser = req.user;
            let isScopeRestricted = false;
            let depotIds = [];
            if (reqUser && !(0, permissions_config_1.isAdminRole)(reqUser.role)) {
            ...
            if (salesperson_id) {
                whereUsers.id = parseInt(salesperson_id);
            }
            if (depot_id) {
                whereUsers.depot_id = parseInt(depot_id);
            }
            if (zone_id) {
                whereUsers.zone_id = parseInt(zone_id);
            }
            if (isScopeRestricted) {
                if (depotIds.length > 0) {
            ...
            // Fetch Visit Tasks
            const visitTasks = await prisma_client_1.default.visit_tasks.findMany({
                where: {
                    is_active: 'Y',
                },
                include: {
            ...
            const returns = await prisma_client_1.default.return_requests.findMany({
                where: {
                    is_active: 'Y',
                    ...(Object.keys(dateFilter).length > 0 && {
            ...
            const serializedReps = users.map(user => {
                ...
                const repTasks = visitTasks.filter(t => t.visit_tasks_visits?.sales_person_id === user.id);
                const completedTasks = repTasks.filter(t => t.status === 'completed').length;
                const repOrders = orders.filter(o => o.salesperson_id === user.id);
                const repInvoices = invoices.filter(i => i.orders?.salesperson_id === user.id);
                const repPayments = payments.filter(p => p.collected_by === user.id);
                const repReturns = returns.filter(r => r.createdby === user.id);
                const visitDuration = repVisits.reduce((sum, v) => sum + (v.duration || 0), 0);
                const avgVisitDuration = repVisits.length > 0 ? visitDuration / repVisits.length : 0;
                const orderValue = repOrders.reduce((sum, o) => sum + Number(o.total_amount || 0), 0);
                const invoiceValue = repInvoices.reduce((sum, i) => sum + Number(i.total_amount || 0), 0);
                const collection = repPayments.reduce((sum, p) => sum + Number(p.total_amount || 0), 0);
            ...
            const completedTasks = visitTasks.filter(t => t.status === 'completed').length;
            const totalOrders = orders.length;
            const totalOrderValue = orders.reduce((sum, o) => sum + Number(o.total_amount || 0), 0);
            const totalCollection = payments.reduce((sum, p) => sum + Number(p.total_amount || 0), 0);
            const summary = {
     ...
     */
    async exportRepProductivityReport(req, res) {
        try {
            const { start_date, end_date, salesperson_id, depot_id, zone_id } = req.query;
            // Build date filter
            const dateFilter = {};
            ...
            // Set response headers
            res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
            res.setHeader('Content-Disposition', `attachment; filename=Rep_Productivity_Report_${Date.now()}.xlsx`);
            res.setHeader('Content-Length', buffer.byteLength.toString());
            res.send(Buffer.from(buffer));
        }
        catch (error) {
     ...
     */
    async getCompetitorAnalysisReport(req, res) {
        try {
            ...
            if (customer_id) {
                whereFilter.customer_id = parseInt(customer_id);
            }
            if (brand_name) {
            ...
            const competitorActivities = await prisma_client_1.default.competitor_activity.findMany({
                where: whereFilter,
                include: {
            ...
            const serializedActivities = competitorActivities.map(activity => ({
                ...
                customer_address: activity.competitor_activity_customers?.address || 'N/A',
            ...
            const brandsSummary = Array.from(new Set(serializedActivities.map(a => a.brand_name))).map(brand => {
                const brandActivities = serializedActivities.filter(a => a.brand_name === brand);
                const avgPrice = brandActivities.reduce((sum, a) => sum + a.observed_price, 0) /
                    brandActivities.length;
                const avgVisibility = brandActivities.reduce((sum, a) => sum + a.visibility_score, 0) /
                    brandActivities.length;
                return {
                    brand_name: brand,
            ...
            const summary = {
                ...
                unique_customers: new Set(serializedActivities.map(a => a.customer_name)).size,
                avg_visibility_score: serializedActivities.reduce((sum, a) => sum + a.visibility_score, 0) /
     ...
     */
    async exportCompetitorAnalysisReport(req, res) {
        try {
            const { start_date, end_date, customer_id, brand_name } = req.query;
            const whereFilter = {
                is_active: 'Y',
            ...
            const brandsSummary = Array.from(new Set(competitorActivities.map((a) => a.brand_name))).map(brand => {
                const brandActivities = competitorActivities.filter((a) => a.brand_name === brand);
                const avgPrice = brandActivities.reduce((sum, a) => sum + Number(a.observed_price || 0), 0) / brandActivities.length;
                const avgVisibility = brandActivities.reduce((sum, a) => sum + Number(a.visibility_score || 0), 0) / brandActivities.length;
                return {
                    brand,
            ...
            const summary = [
                ...
                { metric: 'Unique Brands', value: brandsSummary.length },
                {
                    ...
                    value: new Set(competitorActivities.map((a) => a.customer_id))
                        .size,
                },
            ];
            summary.forEach(row => {
            ...
            // Set response headers
            res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
            res.setHeader('Content-Disposition', `attachment; filename=Competitor_Analysis_Report_${Date.now()}.xlsx`);
            res.setHeader('Content-Length', buffer.byteLength.toString());
            res.send(Buffer.from(buffer));
        }
        catch (error) {
     ...
     */
    async getOutstandingCollectionReport(req, res) {
        try {
            ...
            if (start_date || end_date) {
                ...
                if (start_date) {
                    whereInvoices.invoice_date.gte = new Date(start_date);
                }
                if (end_date) {
                    whereInvoices.invoice_date.lte = new Date(end_date);
                }
            }
            const invoices = await prisma_client_1.default.invoices.findMany({
                where: whereInvoices,
            ...
            if (start_date || end_date) {
                ...
                if (start_date) {
                    wherePayments.payment_date.gte = new Date(start_date);
                }
                if (end_date) {
                    wherePayments.payment_date.lte = new Date(end_date);
                }
            }
            const payments = await prisma_client_1.default.payments.findMany({
                where: wherePayments,
            ...
            const outstandingInvoices = invoices
                ...
                customer_name: invoice.invoices_customers?.name || 'N/A',
                customer_code: invoice.invoices_customers?.code || 'N/A',
                salesperson_name: invoice.orders?.orders_salesperson_users?.name || 'N/A',
                salesperson_code: invoice.orders?.orders_salesperson_users.employee_id ||
                    'N/A',
                ...
                days_overdue: invoice.due_date
                    ? Math.max(0, Math.floor((Date.now() - new Date(invoice.due_date).getTime()) /
                        (1000 * 60 * 60 * 24)))
                    : 0,
            }));
            // Aggregate by Customer
            const customerSummary = Array.from(new Set(outstandingInvoices.map(inv => inv.customer_name))).map(customerName => {
                const customerInvoices = outstandingInvoices.filter(inv => inv.customer_name === customerName);
                const totalOutstanding = customerInvoices.reduce((sum, inv) => sum + inv.balance_due, 0);
                return {
                    customer_name: customerName,
                    customer_code: customerInvoices[0]?.customer_code || 'N/A',
                    invoice_count: customerInvoices.length,
                    total_outstanding: totalOutstanding,
                    avg_days_overdue: customerInvoices.reduce((sum, inv) => sum + inv.days_overdue, 0) /
            ...
            const collections = payments.map(payment => ({
                ...
                customer_name: payment.payments_customers?.name || 'N/A',
                customer_code: payment.payments_customers?.code || 'N/A',
                collected_by: payment.users_payments_collected_byTousers?.name || 'N/A',
                amount: Number(payment.total_amount || 0),
                method: payment.method || 'N/A',
                reference_number: payment.reference_number || 'N/A',
            }));
            const summary = {
                total_outstanding_amount: outstandingInvoices.reduce((sum, inv) => sum + inv.balance_due, 0),
                total_outstanding_invoices: outstandingInvoices.length,
                total_customers_with_outstanding: customerSummary.length,
                total_collections: collections.reduce((sum, col) => sum + col.amount, 0),
                total_collection_count: collections.length,
                avg_days_overdue: outstandingInvoices.reduce((sum, inv) => sum + inv.days_overdue, 0) /
     ...
     */
    async exportOutstandingCollectionReport(req, res) {
        try {
            const { start_date, end_date, customer_id, invoice_status } = req.query;
            const whereInvoices = {
                is_active: 'Y',
            ...
            const wherePayments = { is_active: 'Y' };
            if (start_date || end_date) {
                wherePayments.payment_date = {};
                if (start_date) {
                    wherePayments.payment_date.gte = new Date(start_date);
            ...
            const summary = [
                {
                    ...
                    value: outstandingInvoices.reduce((sum, inv) => sum + Number(inv.balance_due || 0), 0),
                },
                { metric: 'Outstanding Invoices', value: outstandingInvoices.length },
                {
                    metric: 'Total Collections',
                    value: payments.reduce((sum, p) => sum + Number(p.total_amount || 0), 0),
                },
            ...
            // Set response headers
            res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
            res.setHeader('Content-Disposition', `attachment; filename=Outstanding_Collection_Report_${Date.now()}.xlsx`);
            res.setHeader('Content-Length', buffer.byteLength.toString());
            res.send(Buffer.from(buffer));
        }
        catch (error) {
     ...
     */
    async getAttendanceHistoryReport(req, res) {
        try {
            const { page = '1', limit = '10', start_date, end_date, user_id, action_type, search, } = req.query;
            const reqUser = req.user;
            let isScopeRestricted = false;
            let depotIds = [];
            if (reqUser && !(0, permissions_config_1.isAdminRole)(reqUser.role)) {
            ...
            const { data, pagination } = await (0, paginate_1.paginate)({
                model: prisma_client_1.default.attendance_history,
                filters,
                page: pageNum,
                limit: limitNum,
            ...
            const [totalHistory, punchInCount, punchOutCount, historyThisMonth] = await Promise.all([
                prisma_client_1.default.attendance_history.count({
                    where: { is_active: 'Y' },
                }),
                prisma_client_1.default.attendance_history.count({
                    where: {
            ...
            const serializedData = data.map((history) => ({
                id: history.id,
                ...
                attendance: history.attendance_historys
                    ? {
                        ...
                        attendance_date: history.attendance_historys.attendance_date?.toISOString() ||
                            null,
                        user: history.attendance_historys.attendance_user
                            ? {
                                ...
                                employee_id: history.attendance_historys.attendance_user.employee_id,
                                profile_image: history.attendance_historys.attendance_user.profile_image,
                            }
                            : null,
                    }
                    : null,
     ...
     */
    async exportAttendanceHistoryReport(req, res) {
        try {
            ...
            const historyRecords = await prisma_client_1.default.attendance_history.findMany({
                where: filters,
                include: {
                    attendance_historys: {
                        include: {
            ...
            historyRecords.forEach((history) => {
                ...
                historySheet.addRow({
                    ...
                    attendance_date: history.attendance_historys?.attendance_date
                        ? new Date(history.attendance_historys.attendance_date).toLocaleDateString()
                        : 'N/A',
            ...
            const buffer = await workbook.xlsx.writeBuffer();
            res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
            res.setHeader('Content-Disposition', `attachment; filename=Attendance_History_Report_${Date.now()}.xlsx`);
            res.setHeader('Content-Length', buffer.byteLength.toString());
            res.send(Buffer.from(buffer));
        }
        catch (error) {
     ...
     */
    async getCoolerInspectionsReport(req, res) {
        try {
            const { page, limit, barcode, customer_name, is_working, action_required, status, inspector_id, inspection_date, } = req.query;
            ...
            if (barcode) {
                where.coolers = {
                    OR: [
                        ...
                        {
                            cooler_asset_master: { barcode: { contains: barcode } },
                        },
                    ],
                };
            }
            ...
            if (inspector_id) {
                where.inspected_by = parseInt(inspector_id, 10);
            }
            if (inspection_date) {
                const targetDate = new Date(inspection_date);
            ...
            const totalCount = await prisma_client_1.default.cooler_inspections.count({ where });
            const workingCount = await prisma_client_1.default.cooler_inspections.count({
                where: { ...where, is_working: 'Y' },
            });
            const notWorkingCount = await prisma_client_1.default.cooler_inspections.count({
                where: { ...where, is_working: 'N' },
            });
            const actionRequiredCount = await prisma_client_1.default.cooler_inspections.count({
     ...
     */
    async exportCoolerInspectionsReport(req, res) {
        try {
            const { barcode, customer_name, is_working, action_required, status, inspector_id, inspection_date, } = req.query;
            const where = {};
            if (barcode) {
                where.coolers = {
                    OR: [
            ...
            res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
            res.setHeader('Content-Disposition', 'attachment; filename=Cooler_Inspections_Report.xlsx');
     ...
     */
    async getCoolersReport(req, res) {
        try {
            const { page, limit, barcode, customer_name, is_working, action_required, status, inspector_id, inspection_date, } = req.query;
            const page_num = page ? parseInt(page, 10) : 1;
            const limit_num = limit ? parseInt(limit, 10) : 10;
            ...
            if (customer_name) {
                andConditions.push({
                    OR: [
                        { current_location: { contains: customer_name } },
                        {
                            cooler_asset_master: {
                                some: {
                                    coolers_customers: {
            ...
            if (inspector_id) {
                inspectionWhere.inspected_by = parseInt(inspector_id, 10);
            ...
            const mappedAssets = assets.map((a) => {
                const deployedCooler = a.cooler_asset_master && a.cooler_asset_master.length > 0
                    ? a.cooler_asset_master[0]
                    : null;
                const latestInspection = deployedCooler?.cooler_inspections &&
                ...
                return {
                    ...
                    brand: a.asset_master_brands?.name ||
                    ...
                    install_date: deployedCooler?.install_date || a.installation_date || null,
            ...
            const totalCount = await prisma_client_1.default.asset_master.count({ where });
            const baseWhere = {
                asset_master_asset_types: { name: 'Cooler' },
            };
            const deployedCount = await prisma_client_1.default.asset_master.count({
                where: {
                    ...baseWhere,
                    cooler_asset_master: { some: {} },
                },
            });
            const workingCount = await prisma_client_1.default.asset_master.count({
                where: {
                    ...baseWhere,
                    cooler_asset_master: {
                        some: {
            ...
            const actionRequiredCount = await prisma_client_1.default.asset_master.count({
                where: {
                    ...baseWhere,
                    cooler_asset_master: {
                        some: {
    ...
    async exportCoolersReport(req, res) {
        try {
            const { barcode, customer_name, is_working, action_required, status, inspector_id, inspection_date, } = req.query;
            const where = {
                asset_master_asset_types: { name: 'Cooler' },
            ...
            const allAssets = [];
            while (hasMore) {
                const chunk = await prisma_client_1.default.asset_master.findMany({
                    where,
                    skip: offset,
                    take: limit_num,
                    orderBy: { createdate: 'desc' },
            ...
            assets.forEach((row) => {
                const deployedCooler = row.cooler_asset_master && row.cooler_asset_master.length > 0
                ...
                const latestInspection = deployedCooler?.cooler_inspections &&
                    ...
                    ? deployedCooler.cooler_inspections[0]
                    : null;
                const isWorking = latestInspection?.is_working === 'Y'
                    ? 'Working'
                    : latestInspection?.is_working === 'N'
                        ...
                        : deployedCooler?.status === 'working'
                            ...
                            : deployedCooler?.status
                                ? 'Not Working'
                                : 'N/A';
                const installDate = deployedCooler?.install_date || row.installation_date || null;
                const lastScannedDate = latestInspection?.inspection_date ||
                    deployedCooler?.last_scanned_date ||
                    null;
                sheet.addRow({
                    barcode: row.barcode || '—',
                    serial: row.serial_number || '—',
                    type: deployedCooler?.cooler_types?.name ||
                        row.asset_master_asset_types?.name ||
                        '—',
                    subtype: deployedCooler?.cooler_sub_types?.name ||
                        row.asset_master_asset_sub_types?.name ||
                        '—',
                    brand: row.asset_master_brands?.name ||
                    ...
                    depot_name: !deployedCooler?.coolers_customers
                        ...
                        : '—',
                    depot_outlet_code: deployedCooler?.coolers_customers?.code ||
                        (!deployedCooler?.coolers_customers &&
                            row.asset_master_depot?.code) ||
                        '—',
                    outlet_name: deployedCooler?.coolers_customers?.name ||
                        (!deployedCooler?.coolers_customers &&
                            row.asset_master_depot?.name) ||
                    ...
                    last_scanned_date: lastScannedDate
                        ...
                        : '—',
                    last_scanned_seller_code: latestInspection?.users?.employee_id ||
                        deployedCooler?.users?.employee_id ||
                        '—',
                    last_scanned_seller_name: latestInspection?.users?.name || deployedCooler?.users?.name || '—',
                });
            });
            const headerRow = sheet.getRow(1);
            if (headerRow) {
            ...
            const buffer = await workbook.xlsx.writeBuffer();
            res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
            res.setHeader('Content-Disposition', 'attachment; filename=Cooler_Reports_' + Date.now() + '.xlsx');
            res.setHeader('Content-Length', buffer.byteLength.toString());
            res.send(Buffer.from(buffer));
        }
        catch (error) {
