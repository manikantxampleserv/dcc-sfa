                    ...
                    if (salesmanId && record.product_id && recCreatedate) {
                        ...
                        if (refreshedProductSaleMap.has(reconcSaleKey)) {
                            ...
                            if (freshInvoiceItems.length > 0) {
                                ...
                                logger_1.default.info(`[saveReconciliations] Refreshed sale_qty for item ${id} (product ${record.product_id}): ${saleQty} → ${freshQty} (upper bound: ${reconUpperBound?.toISOString()})`);
