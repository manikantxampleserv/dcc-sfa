...
exports.reconciliationController = {
    ...
    getAllReconciliations: async (req, res) => {
        try {
            ...
            const allMatching = await prisma_client_1.default.reconciliation.findMany({
                where: reconcFilters,
            ...
            if (latestOnly) {
                ...
                finalIds = allMatching.map((r) => r.id);
            }
            ...
            const salesman_date_pairs = data
                .filter((rec) => rec.salesman_id && rec.reconciliation_date)
                .map((rec) => ({
                salesman_id: rec.salesman_id,
                rec_date: new Date(rec.reconciliation_date),
            }));
            const loadDateMap = new Map();
            if (salesman_date_pairs.length > 0) {
                const uniqueSalesmanIds = [
                    ...new Set(salesman_date_pairs.map((p) => p.salesman_id)),
                ];
                const vanLoads = await prisma_client_1.default.van_inventory.findMany({
                    where: {
                        user_id: { in: uniqueSalesmanIds },
                        loading_type: 'L',
                        approval_status: 'A',
                        is_cancelled: 'N',
                    },
                ...
                for (const pair of salesman_date_pairs) {
                    ...
                    if (loadDateMap.has(key))
                        continue;
                    const match = vanLoads
                        .filter((v) => v.user_id === pair.salesman_id &&
                        v.document_date &&
                        v.document_date.toISOString().split('T')[0] <= recDateStr)
                        .sort((a, b) => new Date(b.document_date).getTime() -
                        new Date(a.document_date).getTime())[0];
    ...
    getReconciliationById: async (req, res) => {
        try {
            ...
            const items = (reconciliation.reconciliation_items ?? []).map((item) => ({
                ...
                expectedBaseQty: item.expected_base_qty !== null
    ...
    //               //
    //               // IMPORTANT: Use `recCreatedate` (the time the reconciliation was
    //               // generated by the unload button) as the UPPER bound — NOT todayEnd.
    //               // This prevents invoices created *after* the unload was triggered
    //               // (e.g. from a subsequent load + sale session) from being counted
    //               // in this reconciliation's sale_qty, which would make sale_qty
    //               // exceed load_qty and produce incorrect variance.
    //               const reconUpperBound = recCreatedate;
    ...
    //                     ],
    //                     createdate: { gte: sessionStart, lt: reconUpperBound },
    //                     is_active: 'Y',
    ...
    //                 );
    //                 logger.info(
    //                   `[saveReconciliations] Refreshed sale_qty for item ${id} (product ${record.product_id}): ${saleQty} → ${freshQty} (upper bound: ${reconUpperBound?.toISOString()})`
    //                 );
                    ...
                    if (record.sale_qty === null &&
                        ...
                        const batchMovements = await tx.stock_movements.findMany({
                            where: {
                                ...
                                createdate: { gte: sessionStart, lt: reconUpperBound },
                                ...(record.batch_number
                                    ? { batch_lots: { batch_number: record.batch_number } }
                                    : {}),
                            },
                            select: { quantity: true, base_quantity: true },
                        });
                        if (batchMovements.length > 0) {
                            ...
                            await tx.reconciliation_items.update({
                                where: { id },
                                data: { sale_qty: saleQty, sale_base_qty: saleBaseQty },
                            });
                        }
                    }
                    const freshExpectedQty = loadQty - saleQty;
