...
exports.vanInventoryController = {
                            ...
                            if (trackingType === 'BATCH') {
                                ...
                                await tx.van_inventory_items.update({
                                    ...
                                    data: {
                                        ...
                                        total_amount: (vanItem.quantity - qty) *
                                            Number(vanItem.unit_price || 0),
                                    },
                                });
                                const inventoryStock = await tx.inventory_stock.findFirst({
    ...
    //                     );
    //                   // Only update inventory_stock, not van_inventory_items, batch_lots, or product_batches
    //                   const inventoryStock = await tx.inventory_stock.findFirst({
    //                     where: {
    //                       product_id: product.id,
    //                       location_id: inventoryData.location_id || 1,
    ...
    //                     to_location_id: null,
    //                     quantity: batchQty,
    //                     remarks: `Unloaded batch ${batchLot.batch_number} from van`,
    //                     van_inventory_id: inventory.id,
    //                     createdby: userId,
    //                   });
    //                 }
    ...
    //                   if (!existingSerial)
    //                     throw new Error(`Serial ${serialNumber} not found`);
    //                   // Only update inventory_stock, not van_inventory_items or serial_numbers
    //                   const inventoryStock = await tx.inventory_stock.findFirst({
    //                     where: {
    //                       product_id: product.id,
    //                       serial_number_id: existingSerial.id,
    ...
    //                 }
    //               } else {
    //                 // NONE tracking type - only update inventory_stock, not van_inventory_items
    //                 const inventoryStock = await tx.inventory_stock.findFirst({
    //                   where: {
    //                     product_id: product.id,
    //                     location_id: inventoryData.location_id || 1,
    ...
    async getSalespersonInventory(req, res) {
        try {
            const { salesperson_id } = req.params;
            const { page, limit, product_id, document_date, loading_type, include_expired_batches = 'false', batch_status, serial_status, } = req.query;
            const pageNum = parseInt(page, 10) || 1;
            const limitNum = parseInt(limit, 10) || 50;
    ...
    async unloadVanInventory(req, res) {
        try {
            ...
            for (const vanInventory of vanInventories) {
                try {
                    console.log(`Processing van inventory ${vanInventory.id} with ${vanInventory.van_inventory_items_inventory.length} items`);
                    await prisma_client_1.default.$transaction(async (tx) => {
                        const stockMovementData = [];
                        const serialStockIds = [];
                        const batchStockIds = [];
                        const vanInventoryItemIds = [];
                        const batchLotIds = [];
                        ...
                        for (const item of vanInventory.van_inventory_items_inventory) {
                            ...
                            try {
                                const trackingType = product?.tracking_type?.toLowerCase() || 'none';
                                vanInventoryItemIds.push(item.id);
                                if (trackingType === 'serial') {
                                    const serialStockRecords = await tx.inventory_stock.findMany({
                                        ...
                                        select: { id: true, serial_number_id: true },
                                    });
                                    serialStockIds.push(...serialStockRecords.map(r => r.id));
                                    serialNumberIds.push(...serialStockRecords
                                        .map(r => r.serial_number_id)
                                        .filter((id) => id !== null));
                                }
                                else {
                                    const whereClause = {
                        ...
                        await Promise.all([
                            ...
                            tx.van_inventory_items.updateMany({
                                where: { id: { in: vanInventoryItemIds } },
                                data: {
                                    quantity: 0,
                                },
                            }),
                        ]);
                        for (const batchLot of batchLotIds) {
                            await tx.batch_lots.update({
                                where: { id: batchLot.id },
                                data: {
                                    quantity: {
                                        decrement: batchLot.quantity,
                                    },
                                    remaining_quantity: {
                                        decrement: batchLot.quantity,
                                    },
                        ...
                        if (serialNumberIds.length > 0) {
                            await tx.serial_numbers.updateMany({
                        ...
                        const vanInventoryItemsData = vanInventory.van_inventory_items_inventory.map(item => ({
                ...
                catch (vanInventoryError) {
                    console.error(`Failed to process van inventory ${vanInventory.id}:`, vanInventoryError);
                    errors.push(`Van Inventory ${vanInventory.id}: ${vanInventoryError.message}`);
                    continue;
                }
            }
            return res.json({
