  ...
  for (const promo of promotions) {
    ...
    if (
      ...
      if (
        ...
        for (const cat of promo.promotion_customer_category_promotions) {
          const category =
            await prisma_client_1.default.customer_category.findUnique({
    ...
    for (const condition of promo.promotion_condition_promotions) {
      ...
      if (applicableLevel.discount_type === 'PERCENTAGE') {
        ...
        discountAmount = new client_1.Prisma.Decimal(
          applicableLevel.discount_value || 0
        );
      }
      const freeProducts = [];
...
exports.ordersController = {
  async createOrUpdateOrder(req, res) {
    ...
    try {
      ...
      for (const item of items) {
        const itemTotal = new client_1.Prisma.Decimal(item.quantity).mul(
          new client_1.Prisma.Decimal(item.price || item.unit_price || 0)
        );
        ...
        try {
          ...
          if (
            ...
            if (
              ...
              const categories =
                await prisma_client_1.default.customer_category.findMany({
                  where: {
                    id: { in: categoryIds },
          ...
          for (const item of items) {
            ...
            if (productMatch) {
              ...
              const linePrice = new client_1.Prisma.Decimal(
          ...
          const minValue = new client_1.Prisma.Decimal(
      ...
      const shipping_amount = new client_1.Prisma.Decimal(
      ...
      if (result && !orderId) {
        try {
          await (0, helpers_1.createOrderNotification)(
  ...
  async approveOrRejectOrder(req, res) {
    try {
      ...
      const workflow =
        await prisma_client_1.default.approval_workflows.findFirst({
          where: {
            reference_type: 'order',
          ...
          if (
            ...
            if (!existingWorkflow) {
              try {
                ...
                helpers_1.createOrderApprovalWorkflow)(
                  result.id,
                  result.order_number || '',
                  orderCreatorId,
                  priority,
                ...
                if (workflow) {
                  if (result.salesperson_id) {
                    ...
                    for (const approverId of approvers) {
                      await (0, helpers_1.createWorkflowNotification)(
                        approverId,
                        workflow.id,
                        result.order_number || '',
                        'pending',
  ...
  async deleteOrders(req, res) {
    try {
      ...
      if (!existingOrder)
        return res.status(404).json({ message: 'Order not found' });
      await prisma_client_1.default.orders.delete({
        where: { id: Number(id) },
      });
