...
interface ImportDeliveryScheduleProps {
  drawerOpen: boolean;
...
const ImportDeliverySchedule: React.FC<ImportDeliveryScheduleProps> = ({
  drawerOpen,
  setDrawerOpen,
}) => {
  ...
  const handleDownloadSample = async () => {
    try {
      await downloadTemplateMutation.mutateAsync('delivery_schedules');
    } catch (error) {
  ...
  return (
    <CustomDrawer
      ...
      title="Import Delivery Schedules"
      size="large"
    >
      <Box className="!p-5">
        <form onSubmit={formik.handleSubmit} className="!space-y-6">
          <Alert severity="info" className="!mb-4">
            <Typography variant="body2">
              Upload an Excel file to import multiple delivery schedules.
...
export default ImportDeliverySchedule;
