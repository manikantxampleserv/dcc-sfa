...
interface ImportCoolerInspectionProps {
  drawerOpen: boolean;
...
const ImportCoolerInspection: React.FC<ImportCoolerInspectionProps> = ({
  drawerOpen,
  setDrawerOpen,
}) => {
  ...
  const handleDownloadSample = async () => {
    try {
      await downloadTemplateMutation.mutateAsync('cooler_inspections');
    } catch (error) {
  ...
  return (
    <CustomDrawer
      ...
      title="Import Cooler Inspections"
      size="large"
    >
      <Box className="!p-5">
        <form onSubmit={formik.handleSubmit} className="!space-y-6">
          <Alert severity="info" className="!mb-4">
            <Typography variant="body2">
              Upload an Excel file to import multiple cooler inspections.
...
export default ImportCoolerInspection;
