...
interface ImportCreditNoteProps {
  drawerOpen: boolean;
...
const ImportCreditNote: React.FC<ImportCreditNoteProps> = ({
  drawerOpen,
  setDrawerOpen,
}) => {
  ...
  const handleDownloadSample = async () => {
    try {
      await downloadTemplateMutation.mutateAsync('credit_notes');
    } catch (error) {
        ...
        <form onSubmit={formik.handleSubmit} className="!space-y-6">
          <Alert severity="info" className="!mb-4">
            <Typography variant="body2">
              Upload an Excel file to import multiple credit notes. Download the
...
export default ImportCreditNote;
