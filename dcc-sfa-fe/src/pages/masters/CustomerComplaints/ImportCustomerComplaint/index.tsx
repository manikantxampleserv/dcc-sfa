...
interface ImportCustomerComplaintProps {
  drawerOpen: boolean;
...
const ImportCustomerComplaint: React.FC<ImportCustomerComplaintProps> = ({
  drawerOpen,
  setDrawerOpen,
}) => {
  ...
  const handleDownloadSample = async () => {
    try {
      await downloadTemplateMutation.mutateAsync('customer_complaints');
    } catch (error) {
  ...
  return (
    <CustomDrawer
      ...
      title="Import Customer Complaints"
      size="large"
    >
      <Box className="!p-5">
        <form onSubmit={formik.handleSubmit} className="!space-y-6">
          <Alert severity="info" className="!mb-4">
            <Typography variant="body2">
              Upload an Excel file to import multiple customer complaints.
...
export default ImportCustomerComplaint;
