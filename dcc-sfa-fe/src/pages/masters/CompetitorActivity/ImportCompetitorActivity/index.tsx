...
interface ImportCompetitorActivityProps {
  drawerOpen: boolean;
...
const ImportCompetitorActivity: React.FC<ImportCompetitorActivityProps> = ({
  drawerOpen,
  setDrawerOpen,
}) => {
  ...
  const handleDownloadSample = async () => {
    try {
      await downloadTemplateMutation.mutateAsync('competitor_activity');
    } catch (error) {
  ...
  return (
    <CustomDrawer
      ...
      title="Import Competitor Activities"
      size="large"
    >
      <Box className="!p-5">
        <form onSubmit={formik.handleSubmit} className="!space-y-6">
          <Alert severity="info" className="!mb-4">
            <Typography variant="body2">
              Upload an Excel file to import multiple competitor activities.
...
export default ImportCompetitorActivity;
