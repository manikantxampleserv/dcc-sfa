...
interface ImportAssetMaintenanceProps {
  drawerOpen: boolean;
...
const ImportAssetMaintenance: React.FC<ImportAssetMaintenanceProps> = ({
  drawerOpen,
  setDrawerOpen,
}) => {
  ...
  const handleDownloadSample = async () => {
    try {
      await downloadTemplateMutation.mutateAsync('asset_maintenance');
    } catch (error) {
  ...
  return (
    <CustomDrawer
      ...
      title="Import Asset Maintenance"
      size="large"
    >
      <Box className="!p-5">
        <form onSubmit={formik.handleSubmit} className="!space-y-6">
          <Alert severity="info" className="!mb-4">
            <Typography variant="body2">
              Upload an Excel file to import multiple asset maintenance records.
...
export default ImportAssetMaintenance;
