...
interface ImportProductTargetGroupProps {
  drawerOpen: boolean;
...
const ImportProductTargetGroup: React.FC<ImportProductTargetGroupProps> = ({
  drawerOpen,
  setDrawerOpen,
}) => {
  ...
  const handleDownloadSample = async () => {
    try {
      await downloadTemplateMutation.mutateAsync('product_target_group');
    } catch (error) {
  ...
  return (
    <CustomDrawer
      ...
      title="Import Product Target Groups"
      size="large"
    >
      <Box className="!p-5">
        <form onSubmit={formik.handleSubmit} className="!space-y-6">
          <Alert severity="info" className="!mb-4">
            <Typography variant="body2">
              Upload an Excel file to import multiple product target groups.
...
export default ImportProductTargetGroup;
