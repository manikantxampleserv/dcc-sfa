...
interface ImportAssetBrandProps {
  drawerOpen: boolean;
...
const ImportAssetBrand: React.FC<ImportAssetBrandProps> = ({
  drawerOpen,
  setDrawerOpen,
}) => {
  ...
  const handleDownloadSample = async () => {
    try {
      await downloadTemplateMutation.mutateAsync('asset_brands');
    } catch (error) {
  ...
  return (
    <CustomDrawer
      ...
      title="Import Asset Brands"
    >
      <Box className="!p-5">
        <form onSubmit={formik.handleSubmit} className="!space-y-6">
          <Alert severity="info" className="!mb-4">
            <Typography variant="body2">
              Upload an Excel file to import multiple asset brands. Download the
...
export default ImportAssetBrand;
