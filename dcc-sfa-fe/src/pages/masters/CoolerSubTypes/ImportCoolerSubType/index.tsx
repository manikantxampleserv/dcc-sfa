...
interface ImportCoolerSubTypeProps {
  drawerOpen: boolean;
...
const ImportCoolerSubType: React.FC<ImportCoolerSubTypeProps> = ({
  drawerOpen,
  setDrawerOpen,
}) => {
  ...
  const handleDownloadSample = async () => {
    try {
      await downloadTemplateMutation.mutateAsync('cooler_sub_types');
    } catch (error) {
  ...
  return (
    <CustomDrawer
      ...
      title="Import Cooler Sub Types"
    >
      <Box className="!p-5">
        <form onSubmit={formik.handleSubmit} className="!space-y-6">
          <Alert severity="info" className="!mb-4">
            <Typography variant="body2">
              Upload an Excel file to import multiple cooler sub types. Download
...
export default ImportCoolerSubType;
