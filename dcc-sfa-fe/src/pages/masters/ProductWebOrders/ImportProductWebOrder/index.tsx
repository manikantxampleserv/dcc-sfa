...
interface ImportProductWebOrderProps {
  drawerOpen: boolean;
...
const ImportProductWebOrder: React.FC<ImportProductWebOrderProps> = ({
  drawerOpen,
  setDrawerOpen,
}) => {
  ...
  const handleDownloadSample = async () => {
    try {
      await downloadTemplateMutation.mutateAsync('product_web_order');
    } catch (error) {
  ...
  return (
    <CustomDrawer
      ...
      title="Import Product Web Orders"
      size="large"
    >
      <Box className="!p-5">
        <form onSubmit={formik.handleSubmit} className="!space-y-6">
          <Alert severity="info" className="!mb-4">
            <Typography variant="body2">
              Upload an Excel file to import multiple product web orders.
...
export default ImportProductWebOrder;
