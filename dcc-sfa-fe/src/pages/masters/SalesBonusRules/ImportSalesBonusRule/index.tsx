...
interface ImportSalesBonusRuleProps {
  drawerOpen: boolean;
...
const ImportSalesBonusRule: React.FC<ImportSalesBonusRuleProps> = ({
  drawerOpen,
  setDrawerOpen,
}) => {
  ...
  const handleDownloadSample = async () => {
    try {
      await downloadTemplateMutation.mutateAsync('sales_bonus_rules');
    } catch (error) {
  ...
  return (
    <CustomDrawer
      ...
      title="Import Sales Bonus Rules"
      size="large"
    >
      <Box className="!p-5">
        <form onSubmit={formik.handleSubmit} className="!space-y-6">
          <Alert severity="info" className="!mb-4">
            <Typography variant="body2">
              Upload an Excel file to import multiple sales bonus rules.
...
export default ImportSalesBonusRule;
