...
interface ImportProductShelfLifeProps {
  drawerOpen: boolean;
...
const ImportProductShelfLife: React.FC<ImportProductShelfLifeProps> = ({
  drawerOpen,
  setDrawerOpen,
}) => {
  ...
  const handleDownloadSample = async () => {
    try {
      await downloadTemplateMutation.mutateAsync('product_shelf_life');
    } catch (error) {
  ...
  return (
    <CustomDrawer
      ...
      title="Import Product Shelf Life"
      size="large"
    >
      <Box className="!p-5">
        <form onSubmit={formik.handleSubmit} className="!space-y-6">
          <Alert severity="info" className="!mb-4">
            <Typography variant="body2">
              Upload an Excel file to import multiple product shelf life.
...
export default ImportProductShelfLife;
