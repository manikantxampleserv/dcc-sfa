// ════════════════════════════════════════════════════════════
// SALES CONTROL TOWER — Aggregation Utilities
// ════════════════════════════════════════════════════════════
import type { SalesRow } from './mockData';

export interface AggBucket { UC: number; PC: number; TV: number; count: number; }
...
function blank(): AggBucket { return { UC: 0, PC: 0, TV: 0, count: 0 }; }
...
export function aggregate(rows: SalesRow[]): AggResult {
  const r: AggResult = {
    byDepot: {}, byCoord: {}, bySup: {}, byRoute: {}, bySal: {},
    byBrand: {}, byPack: {}, byCh: {}, bySKU: {}, byDate: {},
    totalUC: 0, totalPC: 0, totalTV: 0, newOutlets: 0, avgStrike: 0, rows,
  };
  let strikeSum = 0;
...
export type FilterState = {
  depot: string; coord: string; sup: string; route: string;
  sal: string; brand: string; pack: string; ch: string;
  dateMode: 'all' | '7d' | '15d' | 'today';
};

export function filterRows(rows: SalesRow[], f: FilterState): SalesRow[] {
  // Dynamically compute the "now" from the latest date in the dataset
  const maxDate = rows.reduce((max, r) => r.Date > max ? r.Date : max, '1970-01-01');
...
export const FMTn = (n: number) => new Intl.NumberFormat().format(Math.round(n));

export const pct = (a: number, b: number) =>
  b ? (((a || 0) - (b || 0)) / Math.abs(b) * 100) : 0;

export const PAL = [
  '#e31837','#1a56db','#059669','#d97706','#7c3aed',
  '#0891b2','#db2777','#ea580c','#16a34a','#dc2626','#2563eb','#9333ea',
];
